...
### [1] [Non-stationary noise in gravitational wave analyses: The wavelet domain noise covariance matrix](https://arxiv.org/abs/2511.10632)
*Neil J. Cornish*

Main category: gr-qc

TL;DR: The paper analyzes the suitability of the Wilson-Daubechies-Meyer (WDM) wavelet packet basis for handling non-stationary noise in gravitational wave data, finding that the diagonal noise covariance approximation holds when noise variations are slow in both time and frequency. Off-diagonal terms depend on derivatives of the spectral model and strategies for rapidly varying noise are outlined.


<details>
  <summary>Details</summary>
Motivation: To determine conditions under which the WDM wavelet domain's diagonal noise covariance matrix approximation remains valid for non-stationary noise analysis, crucial for efficient gravitational wave data processing.

Method: The authors investigated the WDM wavelet packet basis by examining the noise correlation matrix under varying time and frequency conditions, deriving dependencies of off-diagonal terms on noise's time and frequency derivatives.

Result: The diagonal approximation holds when noise varies slowly across wavelet pixels in both time and frequency. Off-diagonal elements are proportional to derivatives of the dynamic spectral model, validating wavelet use for slowly varying noise.

Conclusion: WDM wavelets are suitable for non-stationary noise with slow variations; however, alternative strategies are needed for rapid noise changes. The findings apply broadly to wavelet transforms with compact time-frequency filters.
