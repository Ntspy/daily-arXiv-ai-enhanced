<div id=toc></div>

# Table of Contents

- [gr-qc](#gr-qc) [Total: 17]
- [astro-ph.HE](#astro-ph.HE) [Total: 18]
- [hep-ph](#hep-ph) [Total: 20]
- [astro-ph.IM](#astro-ph.IM) [Total: 7]


<div id='gr-qc'></div>

# gr-qc [[Back]](#toc)

### [1] [Probing for primordial black hole candidates in exoplanet search data](https://arxiv.org/abs/2512.03118)
*Paul Halpern,Erik Cauley,Max Stoltzmann,Mauritz Wilshusen*

Main category: gr-qc

TL;DR: The paper investigates whether certain exoplanet candidates detected via radial velocity and gravitational microlensing could actually be primordial black holes (PBHs), ruling out those detectable via direct imaging or transit methods. Potential PBH candidates include Kepler-21 Ac, HD 219134 f, and microlensing events like MOA 2009-BLG-387L.


<details>
  <summary>Details</summary>
Motivation: PBHs, thought to have masses similar to planets but evade direct detection, might be identifiable via stellar wobbles (radial velocity) or microlensing if captured by stars.

Method: Analyzed data from radial velocity and microlensing surveys, excluding candidates detectable by direct imaging or transit to isolate potential PBHs.

Result: Identified specific candidates like Kepler-21 Ac, HD 219134 b, and microlensing events as possible PBHs but acknowledge uncertainty; emphasizes need for future imaging to distinguish PBHs from exoplanets.

Conclusion: While potential PBH candidates exist, further observations focusing on planetary vs. evaporation signatures are needed to confirm their nature.

Abstract: We have sifted through astrophysical data collected by various radial velocity and gravitational microlensing searches for exoplanets with the goal of identifying potential signs of the presence of primordial black holes (PBH). Our motivation is that those hypothesized remnants of inhomogeneous energetic fluctuations in the early universe, though too small for direct detection, are thought to have a mass range similar to that of planets. Thereby, if captured by stars, they could conceivably make their presence known through stellar wobbles picked up by means of Doppler spectroscopy in the radial velocity method, or alternatively through microlensing. In our analysis of such data, we have identified potential PBH contenders by ruling out any exoplanet candidates that have been detected through direct imaging or transit methods, as they would have sizes consistent with planets rather than PBHs. In particular we focus on the objects Kepler-21 Ac, HD 219134 f, Gliese 686 b, HR 5183 b, HD 20794 e, and Wolf 1061 d, each of which has been found using the radial velocity method but never imaged (either directly or through transit). We also examine the microlensing events MOA 2009-BLG-387L and OGLE-2016-BLG-1540, which offer promise as candidate PBHs. We present these as a representative, but not exclusive list, of potential PBH contenders. Furthermore, future imaging, especially focused on signals of planetary dimensions versus evaporation signatures, might clarify which of these are indeed exoplanets.

</details>


### [2] [Inferring black hole formation channels in GWTC-4.0 via parametric mass-spin correlations derived from first principles](https://arxiv.org/abs/2512.03152)
*Emanuele Berti,Francesco Crescimbeni,Gabriele Franciolini,Simone Mastrogiovanni,Paolo Pani,Grégoire Pierra*

Main category: gr-qc

TL;DR: The study analyzes binary black hole formation scenarios using spin-mass correlations from GWTC-4.0, favoring hierarchical stellar evolution with mass-dependent spin correlations and ruling out primordial BHs for most events.


<details>
  <summary>Details</summary>
Motivation: To distinguish between BBH formation mechanisms (isolated evolution, dynamical clustering, AGN disks, primordial origin) by comparing predicted spin properties with observation data.

Method: Hierarchical Bayesian inference applied to GWTC-4.0 data, modeling spin-mass correlations based on first-principles for each formation scenario.

Result: Data strongly support mass-spin magnitude correlation, favoring hierarchical models (allowing second-generation mergers). AGN and cluster scenarios are indistinguishable in spin orientation. Primordial BHs poorly fit the data, explaining only part of observed mergers.

Conclusion: Hierarchical stellar evolution best explains BBH spin-mass trends, while primordial BHs are unlikely for most events. Spin orientation data remains inconclusive between cluster and AGN channels.

Abstract: We investigate the differences between several proposed formation scenarios for binary black holes (BBHs), including isolated stellar evolution, dynamical assembly in dense clusters and AGN disks, and primordial BHs. Our approach exploits the predicted spin features of each formation channel, and adopts parameterized models of the predicted correlations between the spin magnitudes (and orientations) and mass, inspired by first principles. Using hierarchical Bayesian inference on the recent GWTC-4.0 dataset, we compare these features across all models and assess how well each scenario explains the data. We find that the data strongly favor the presence of a positive correlation between mass and spin magnitude, in agreement with previous studies. Furthermore, the hierarchical scenario provides a better fit to the observations, due to the inclusion of second-generation mergers leading to higher spins at larger masses. The current dataset is not informative enough about spin orientation: the cluster (random orientations) and AGN (aligned orientations) scenarios have comparable Bayesian evidence. Finally, the mass-spin correlation predicted by the primordial scenario gives a poor fit to the data, and this scenario can only account for a subset of the observed events.

</details>


### [3] [From scalar clouds around evaporating black holes to boson star](https://arxiv.org/abs/2512.03155)
*Daniel Neves,João G. Rosa*

Main category: gr-qc

TL;DR: The study explores the first-ever analysis of scalar clouds around evaporating black holes, showing that these clouds can form self-gravitating boson stars if the black hole mass drops below half the cloud's mass. This mechanism could explain detectable micro-boson stars from primordial black holes interacting with dark matter.


<details>
  <summary>Details</summary>
Motivation: To understand how scalar fields behave around evaporating black holes and to investigate potential mechanisms for boson star formation, which has implications for dark matter detection.

Method: The researchers performed simulations of the Schrödinger-Poisson system for non-relativistic, spherically symmetric scalar clouds bound to black holes undergoing adiabatic evaporation.

Result: The simulations demonstrate that a scalar cloud can partially survive as a boson star when the black hole's mass becomes less than half of the cloud's mass, providing a new mechanism for boson star formation.

Conclusion: This study supports the conjecture that light primordial black holes interacting with bosonic dark matter could produce detectable micro-boson stars with large occupation numbers, even for weakly interacting particles.

Abstract: We study, for the first time, the evolution of a scalar cloud bound to an evaporating black hole. Our simulations of the associated Schrödinger-Poisson system for non-relativistic and spherically symmetric clouds reveal that a scalar cloud may (partially) survive as a self-gravitating boson star if the black hole evaporates adiabatically until its mass becomes less than one half of the cloud's mass. This yields a novel mechanism for boson star formation and shows that, as previously conjectured, bosonic dark matter production by light primordial black holes may result in micro-boson stars with very large occupation numbers, greatly enhancing their potential detectability even for very weakly interacting dark matter particles.

</details>


### [4] [Many Worlds in Theory Space: A Quantum Origin for the Constants of Nature](https://arxiv.org/abs/2512.03251)
*Edward J Shaya*

Main category: gr-qc

TL;DR: The paper proposes that fundamental constants in physics arise from quantum mechanics through a Grand Hilbert Space framework, where these constants are part of the universe's wavefunction. Decoherence during the early universe leads to distinct classical universes with varying constants, including our life-permitting one. The authors introduce the Meta-Wheeler-DeWitt equation and a Bayesian evidence metric based on habitable sectors, predicting that Standard Model parameters cannot be derived purely mathematically.


<details>
  <summary>Details</summary>
Motivation: The universe's fine-tuned constants require explanation. This work seeks to address why these values are conducive to life by deriving them from quantum principles rather than assuming them as fixed.

Method: The approach involves expanding quantum cosmology's configuration space into U-sectors within a Grand Hilbert Space. The Meta-Wheeler-DeWitt equation governs parameter dynamics. Decoherence splits possibilities into classical universes. Bayesian metrics compare theories based on their ability to produce habitable sectors.

Result: Derived equations show primordial coherence where physical laws fluctuate. A prediction is made that Standard Model parameters lack purely mathematical derivation. Framework provides tools for evaluating fundamental theories quantitatively.

Conclusion: The theory offers a mechanism for emergent physical constants via quantum processes without requiring fine-tuning. It shifts the foundation of constants from arbitrary values to quantum-mechanical origins, suggesting a multiverse of possible laws and offering testable predictions about derivability of constants.

Abstract: Many of the numbers appearing in the laws of physics, such as the strength of electromagnetism or the masses of elementary particles, must lie in precise ranges for stars, planets, and chemistry to exist. Why the universe has such these values is one of the deepest questions in science. Here we propose that these constants arise from quantum mechanics itself. By enlarging the configuration space of quantum cosmology, we treat the constants of nature as part of the wavefunction of the universe. The universal wavefunction contains support for many possible sets of constants, and early-universe processes cause these possibilities to decohere into distinct classical universes. Our universe is one such branch, compatible with complexity and life. We defined the Grand Hilbert Space as the direct sum of U-sectors, each representing a distinct set of physical laws. We derived the Meta-Wheeler--DeWitt equation, which governs the dynamics of the theory parameters. We showed that in the Planck era, the kinetic terms for these parameters are active, creating a state of primordial coherence where the laws of physics effectively fluctuate. This formalism offers a quantitative tool for high-energy physics. We proposed that the total integrated amplitude over theory space serves as a Bayesian evidence metric, allowing for the statistical comparison of rival microscopic theories based on their fertility in generating habitable sectors. Finally, this work makes a specific, falsifiable prediction: there will never be a successful, purely mathematical derivation of the Standard Model parameters.

</details>


### [5] [Constraints on Reversing the Thermodynamic Arrow of Time from Black Hole Thermodynamics, Wormholes, and Time-Symmetric Quantum Mechanics](https://arxiv.org/abs/2512.03380)
*Kevin Song,John Zhang*

Main category: gr-qc

TL;DR: The paper investigates whether reversing the thermodynamic arrow of time temporarily in a single universe via semiclassical gravity is possible. It concludes that while black holes, wormholes, and retrocausal quantum mechanics might redistribute entropy among sectors, they cannot reverse the universal thermodynamic arrow under current theoretical constraints.


<details>
  <summary>Details</summary>
Motivation: The motivation is to resolve whether entropy decrease can occur in a single universe without multiverse assumptions, exploring if gravitational and quantum effects provide pathways for temporary time arrow reversal.

Method: The authors analyze entropy types (fine-grained, coarse-grained, gravitational), apply the Generalized Second Law, and develop the GET framework with sectoral inequalities to quantify entropy transport across matter, radiation, and gravity sectors in scenarios involving black holes, wormholes, and retrocausal models.

Result: GET framework shows entropy redistribution is possible but not reversal of the universal thermodynamic arrow. Black holes/wormholes only shift entropy between sectors without net decrease in total entropy.

Conclusion: Semiclassical gravity frameworks cannot achieve temporary thermodynamic arrow reversal in a single connected universe due to entropy conservation bounds and sectoral redistribution limits under existing physical constraints.

Abstract: Can the thermodynamic arrow of time in a single universe be reversed, even temporarily, within semiclassical gravity without invoking additional universes or branches? We address this question in a single, connected spacetime where quantum field theory is coupled to classical general relativity, and where black holes, traversable wormholes, and time-symmetric or retrocausal formulations of quantum mechanics might naively appear to open channels for entropy export or cancellation. After distinguishing fine-grained, coarse-grained, and generalized gravitational entropy, and formulating a cosmological coarse-grained entropy, we treat black hole evaporation, wormholes constrained by quantum energy inequalities, and two-time boundary-value frameworks (including absorber-type and two-state-vector formalisms) within a common information-theoretic language. We then introduce a "Global Entropy Transport" (GET) framework and derive a sectoral inequality that bounds the net decrease of matter-plus-radiation entropy in terms of changes in horizon area and correlation (mutual-information) terms, assuming the generalized second law and modern focusing and energy conditions. Within this framework, black holes, wormholes, and retrocausal protocols can at most redistribute entropy among matter, radiation, and gravitational sectors and reshape the local pattern of entropy production. They do not, under current semiclassical, holographic, and statistical-mechanical constraints, permit a genuine reversal of the universal thermodynamic arrow in a single connected universe.

</details>


### [6] [Motion of a charged test particle around a static black hole in a monopole magnetic field](https://arxiv.org/abs/2512.03484)
*Ken-ichi Nakao,Yota Endo,Hideki Ishihara,Kenta Matsuo,Kensuke Sueto,Koudai Ueda,Hirotaka Yoshino*

Main category: gr-qc

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: We study the motion of a charged test particle in the spacetime with a spherically symmetric black hole which is immersed in a monopole magnetic field. We show that the radial motion of the charged test particle is governed by completely the same equation as that in the case of no magnetic field. This result implies that the black hole will acquire the electric charge if it is surrounded by the collisionless plasma composed of protons and electrons which obey the Maxwell velocity distribution. The drastically different situation appears in the tangential motions of charged test particles due to the magnetic field. The trajectory of a charged test particle in the black hole with the magnetic field of the order of 10 Gauss near the black hole, is confined on a very thin cone as long as the specific angular momentum of the particle is not much larger than the gravitational radius of the black hole times the speed of light. This result leads to a possibility that a plasma lump can hover over the black hole and is very hot, in the monopole magnetic field.

</details>


### [7] [Multi-probe analysis of strong-field effects in $f(Q)$ gravity](https://arxiv.org/abs/2512.03529)
*Mohsen Khodadi,Behnam Pourhassan,Emmanuel N. Saridakis*

Main category: gr-qc

TL;DR: The paper tests the strong-field predictions of covariant $f(Q)$ gravity using black-hole shadows, EHT data, S2-star precession, and lensing. Case II solutions show significant deviations from Schwarzschild, while Case I does not. EHT and lensing data impose tight constraints on the theory.


<details>
  <summary>Details</summary>
Motivation: Covariant $f(Q)$ gravity is an extension of General Relativity, but its predictions in strong gravitational fields need testing. The motivation is to use precise astrophysical observations to validate or constrain the theory's applicability.

Method: Analyze static, spherically symmetric black-hole solutions (Case I and II) of covariant $f(Q)$ gravity against observations: shadow sizes from EHT (M87* and Sgr A*), S2-star precession, and strong lensing coefficients. Compare deviations from Schwarzschild solutions.

Result: Case I shows negligible deviations; Case II produces observable effects on photon spheres and shadow sizes. EHT and lensing data provide the tightest constraints yet on $f(Q)$ gravity parameters.

Conclusion: Current EHT and lensing observations strongly constrain covariant $f(Q)$ gravity. Future tests with advanced imaging and Galactic-center measurements could further validate or rule out the theory.

Abstract: Covariant $f(Q)$ gravity is a viable extension of General Relativity, however its strong-field predictions remain largely untested. Using the static, spherically symmetric black-hole solutions of the theory, we confront it with the most stringent probes available: black-hole shadows, Event Horizon Telescope (EHT) measurements, S2-star precession, and strong gravitational lensing. We show that the two admissible solution branches behave very differently: Case~I produces negligible deviations from Schwarzschild solution, whereas Case~II yields significant, potentially observable corrections to the photon sphere and shadow size. From the EHT shadow diameters of M87* and Sgr~A*, we obtain tight bounds, which are further strengthened by strong-lensing coefficients. These results provide the sharpest strong-field constraints on covariant $f(Q)$ gravity to date, and point toward future tests using next-generation horizon-scale imaging and precision Galactic-center astrometry.

</details>


### [8] [Feeding a Kerr black hole with quantized vortices](https://arxiv.org/abs/2512.03561)
*Shilong Jin,Xiaofei Zhao,Yong Zhang,Chi Xiong*

Main category: gr-qc

TL;DR: The paper investigates quantized vortices in quantum fluids near Kerr black holes by solving a nonlinear Klein-Gordon equation in Kerr geometry, revealing new phenomena like vortex-induced flows and their implications for dark matter models, frame-dragging effects, and astrophysical phenomena.


<details>
  <summary>Details</summary>
Motivation: To explore how quantized vortices in quantum fluids (such as axion condensates or ultralight boson clouds) near spinning black holes influence fluid properties and modify dark matter models, while probing general relativistic effects like frame-dragging.

Method: Solving a nonlinear Klein-Gordon equation in Kerr geometry to study vortex formation, dynamics (orbiting, accretion, splitting), and their effects on fluid behavior and spacetime phenomena.

Result: Vortices stably orbit black holes, induce rotational/turbulent flows, form boundary layers near event horizons, and emit energetic outbursts. They quantitatively characterize frame-dragging via angular velocity measurements and link to astrophysical observations (XRISM satellite).

Conclusion: Quantized vortices offer a novel probe for black hole physics, impacting dark matter model revisions, General Relativity validations, and explanations for astrophysical outbursts observed in quantum fluid-dynamics regimes near compact objects.

Abstract: By solving a nonlinear Klein-Gordon equation in Kerr geometry, we uncover new phenomena and key characteristics of quantized vortices in quantum fluids near a Kerr black hole. The formation of these vortices induces rotational or turbulent flows, which profoundly alter the fluid properties and revise those dark matter models describing axion condensates, ultralight boson clouds, and other scalar fields in the vicinity of spinning black holes. As macroscopic, quantum, and topological defects, these vortices can stably orbit the black hole over extended periods, establishing their viability as novel probes for investigating black hole physics. For instance, we calculate the angular velocities of orbiting vortices to quantitatively characterize the frame-dragging effect, a classic prediction of general relativity. Additionally, we observe that relatively large vortices are accreted onto the black hole, wrapping around it while undergoing splitting and reconnecting processes. In quantum fluids with high vortex densities, turbulent flows emerge, accompanied by the formation of a vortex boundary layer near the event horizon. Beyond the ergosphere, we find vortex emissions and energetic outbursts, which may provide crucial insights into analogous astrophysical events recently discovered by the XRISM satellite.

</details>


### [9] [Gauge Symmetries, Contact Reduction, and Singular Field Theories](https://arxiv.org/abs/2512.03645)
*Callum Bell,David Sloan*

Main category: gr-qc

TL;DR: The paper extends the reduction of scale-invariant dynamical systems to singular Lagrangian models for particles and fields using the Hamilton-De Donder-Weyl formalism, leading to frictional theories and applications in General Relativity.


<details>
  <summary>Details</summary>
Motivation: To generalize existing scale-reduction methods to systems with singular Lagrangians, enabling analysis of gauge-invariant theories and their implications for classical field theories like General Relativity.

Method: Using the covariant Hamilton-De Donder-Weyl formalism on pre-multisymplectic velocity phase spaces (J¹E), the authors excise the global scale degree of freedom from singular Lagrangians to derive equivalent frictional theories.

Result: Successful application of the formalism to physically-motivated examples, demonstrating how scale reduction works in singular systems and providing insights into gauge symmetries and energy dissipation in General Relativity.

Conclusion: Scale reduction via/global gauge fixing is universally applicable across classical systems with singular Lagrangians, offering a systematic method for simplifying theories and revealing foundational connections between symmetry, dissipation, and gravitational physics.

Abstract: The reduction of dynamical systems which are invariant under changes of global scale is well-understood, for classical theories of particles, and fields. The excision of the superfluous degree of freedom describing such a scale leads to a dynamically-equivalent theory, which is frictional in nature. In this article, we extend the formalism to physical models, of both particles and fields, described by singular Lagrangians. Our treatment of classical field theory is based on the manifestly covariant Hamilton De-Donder Weyl formalism, in which the Lagrangian density is introduced as a bundle morphism on the pre-multisymplectic velocity phase space $J^1E$. The results obtained are subsequently applied to a number of physically-motivated examples, as well as a discussion presented on the implications of our work for classical General Relativity.

</details>


### [10] [Spherical accretion onto higher-dimensional Reissner-Nordström Black Hole](https://arxiv.org/abs/2512.03668)
*Bibhash Das,Anirban Chanda,Bikash Chandra Paul*

Main category: gr-qc

TL;DR: This paper investigates relativistic spherically symmetric accretion onto higher-dimensional Reissner-Nordström black holes using Hamiltonian dynamics, analyzing two fluid types (isotropic and non-linear polytropic). It finds dimension-dependent critical radii and varying transonic flow behaviors, with the polytropic fluid only showing non-transonic flow. The mass accretion rate peaks differently across dimensions and charge parameters, showing higher rates for charged BHs compared to neutral ones in higher dimensions.


<details>
  <summary>Details</summary>
Motivation: To understand how spacetime dimensions and BH charge influence accretion dynamics, critical points, and mass accretion rates in higher-dimensional Reissner-Nordström black holes through Hamiltonian formalism.

Method: Applied Hamiltonian analysis to model accretion flows with isotropic and polytropic fluids in higher-D RN spacetimes. Derived equations for critical radii, flow behaviors (transonic vs non-transonic), and mass accretion rates across dimensions.

Result: Isotropic fluid exhibits transonic/non-transonic flows; polytropic only non-transonic. Critical radius decreases, bottoms out at D>4, then increases. Maximum mass accretion rate is lowest in Schwarzschild BHs but rises with charge in RN BHs in higher dimensions.

Conclusion: BH charge and dimensionality significantly affect accretion dynamics: charged BHs in higher dimensions show enhanced mass accretion rates, and critical radii exhibit non-monotonic behavior with increasing dimensions.

Abstract: We obtain relativistic solutions of spherically symmetric accretion by a dynamical analysis of a generalised Hamiltonian for higher-dimensional Reissner-Nordström (RN) Black Hole (BH). We consider two different fluids namely, an isotropic fluid and a non-linear polytropic fluid to analyse the critical points in a higher-dimensional RN BH. The flow dynamics of the fluids are studied in different spacetime dimensions in the framework of Hamiltonian formalism. The isotropic fluid is found to have both transonic and non-transonic flow behaviour, but in the case of polytropic fluid, the flow behaviour is found to exhibit only non-transonic flow, determined by a critical point that is related to the local sound speed. The critical radius is found to change with the spacetime dimensions. Starting from the usual four dimensions it is noted that as the dimension increases the critical radius decreases, attains a minimum at a specific dimension ($D>4$) and thereafter increases again. The mass accretion rate for isotropic fluid is determined using Hamiltonian formalism. The maximum mass accretion rate for RN BH with different equations of state parameters is studied in addition to spacetime dimensions. The flow behaviour and mass accretion rate for a change in BH charge is also studied analytically. It is noted that the maximum mass accretion rate in a higher-dimensional Schwarzschild BH is the lowest, which however, increases with the increase in charge parameter in a higher-dimensional RN BH.

</details>


### [11] [A theory-agnostic hierarchical Bayesian framework for black-hole spectroscopy: a case study on GW250114 in Einstein-dilaton-Gauss-Bonnet gravity](https://arxiv.org/abs/2512.03713)
*Shitong Guo,Yan-Gang Miao*

Main category: gr-qc

TL;DR: The paper introduces a theory-agnostic Bayesian framework for black-hole spectroscopy using ringdown observations, applied to GW250114 in Einstein-dilaton-Gauss-Bonnet gravity, showing robust but weak constraints on coupling parameter ζ and highlighting systematic effects from prior choices.


<details>
  <summary>Details</summary>
Motivation: To address model-dependent systematics in current gravitational-wave tests of modified gravity and enable broader theory applicability by comparing observed spectra directly with theoretical quasinormal mode models.

Method: A hierarchical Bayesian framework modeling ringdown signals as damped sinusoids, comparing spectral data with theoretical spectra without assuming specific waveform models, including soft-truncation for parameter validity and stability diagnostics.

Result: Application to GW250114 yielded robust posteriors for ζ but weak discriminatory power across the tested range; injection tests showed recovery of nonzero ζ but potential systematic bias from Kerr-based priors absorbing spectral deviations.

Conclusion: The method provides a transparent, extensible approach for strong-field gravity tests, compatible with next-gen detectors' precision, while emphasizing the need for careful prior selection in modified gravity parameter inference.

Abstract: Black-hole spectroscopy has emerged as a powerful probe of strong-field gravity in the era of gravitational-wave astronomy. In this context, many current tests of modified or extended gravity are implemented by searching for predicted signatures modeled as perturbative corrections to general-relativistic waveforms; however, this approach may introduce model-dependent systematics and limit applicability to broader classes of theories. To complement such methods, we develop a theory-agnostic hierarchical Bayesian framework that connects ringdown observations -- modeled as damped sinusoids -- directly with theoretical quasinormal mode spectra, performing the comparison at the spectral level rather than through theory-specific waveform matching. The framework incorporates a soft-truncation module to account for the finite domain of validity in the theory's parameter space and is equipped with quantitative diagnostics that identify stable analysis time windows. As an illustrative application, we implement the framework within Einstein-dilaton-Gauss-Bonnet gravity and apply it to the gravitational-wave event GW250114, finding that the resulting posterior for the dimensionless coupling $ζ$ is robust against prior assumptions yet remains only weakly informative over the range considered in this work. We further perform controlled ringdown injection studies across different values of $ζ$, confirming that nonzero couplings can be recovered while also indicating a potential systematic effect: Kerr-based priors in the $ζ$ inference may partially absorb spectral deviations arising in alternative theories of gravity. This work establishes a transparent and extensible foundation for future strong-field gravity tests, naturally compatible with the growing precision and modal resolution of next-generation gravitational-wave detectors.

</details>


### [12] [Approximations and modifications of celestial dynamics tested on the three-body system](https://arxiv.org/abs/2512.03823)
*Søren Toxvaerd*

Main category: gr-qc

TL;DR: This paper analyses the impact of PM approximation and MOND vs MOGA modifications on celestial system stability using a three-body system. It finds that PM and MOND destabilize the system due to violated invariances, while MOGA stabilizes it by preserving momentum laws through inverse attraction adjustments for distant objects.


<details>
  <summary>Details</summary>
Motivation: To evaluate the accuracy and long-term effects of common approximations/modifications in celestial dynamics simulations, particularly their impacts on system stability and conservation laws.

Method: Simulating a three-body system (TBS) using particle-mesh (PM), MOND, and MOGA approaches. Testing stability and conservation of momentum/energy in each method.

Result: PM/MOND caused instability due to breaking momentum conservation and Newton's third law. MOGA stabilized the system by preserving these invariances through modified gravity. Errors in PM/MOND, even small, led to dynamic instability.

Conclusion: MOGA is superior for celestial simulations as it maintains critical dynamical invariances. PM/MOND introduce destabilizing errors highlighting the need for gravity modifications that preserve physical laws.

Abstract: Large-scale simulations of celestial systems are based on approximations or modifications of classical dynamics. The approximations are with ``particle-mesh'' (PM) substitutions of the attractions from objects far away, or one modify the Newtonian accelerations (MOND) or the gravities (MOGA). The PM approximation and MOND modification of classical dynamics break the invariances of classical dynamics. The simple three-body system (TBS) is the simplest system to test the approximations and modifications of celestial dynamics, and it is easy to implement on a computer. Simulations of the TBS show that the PM approximation and MOND destabilize TBS. In contrast, the MOGA modification of gravity by replacing Newton's inverse square attraction with an inverse attraction for far-away interactions stabilizes the system. The PM approximation and the MOND modification of classical dynamics do not preserve the momentum and angular momentum of a conservative system exactly, and PM does not obey Newton's third law. Although the errors and shortcomings of these PM approximations and MOND modifications are small, they cause the instability of the regular dynamics.

</details>


### [13] [Some perspective of thermodynamical and optical properties of black holes in Maxwell-dilaton-dRGT-like massive gravity](https://arxiv.org/abs/2512.03832)
*B. Eslam Panah. N. Heidari,M. Soleimani*

Main category: gr-qc

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: Motivated by integrating the dilaton field (as a UV correction) with dRGT-like massive gravity (as an IR correction) into Einstein gravity, we investigate the thermodynamic and optical properties of black holes within this gravitational framework. We begin by reviewing the black hole solutions in Maxwell-dilaton-dRGT-like massive gravity, followed by an analysis of how various parameters influence on the asymptotical behavior of the spacetime and the event horizon of these black holes. In the subsequent section, we examine the conserved and thermodynamic quantities associated with these black holes, paying particular attention to the effects of parameters like $β$, $α$, and the massive parameters ($η_{1}$ and $η_{2}$) on their local stability by simultaneously evaluating the heat capacity and temperature. We also adopt an alternative method to study phase transitions using geometrothermodynamics. Furthermore, we explore how the parameters of Maxwell-dilaton-dRGT-like massive gravity impacts the optical characteristics and radiative behavior of black holes. In particular, we analyze the effects of the dilaton coupling constant ($α$), charge ($q$), the massive gravity parameter ($η_1$), and the graviton mass ($m_g$) on the radius of the photon sphere and the resulting black hole shadow. Moreover, the theoretical shadow radius is compared to the observational data from $Sgr A^*$. Additionally, we investigate the energy emission rate of these black holes, revealing that these parameters substantially influence the emission peak.

</details>


### [14] [Fractional Holographic Dark Energy Driven Reconstruction of $f(Q)$ Gravity and its Cosmological Implications](https://arxiv.org/abs/2512.03855)
*Rajdeep Mazumdar,Kalyan Malakar,Kalyan Bhuyan*

Main category: gr-qc

TL;DR: The paper presents a reconstructed f(Q) gravity model inspired by fractional holographic dark energy, showing strong statistical performance compared to ΛCDM, consistent with observations and classical energy conditions, offering an alternative explanation for cosmic acceleration.


<details>
  <summary>Details</summary>
Motivation: To explain the Universe's late-time acceleration without relying on ΛCDM, by integrating fractional holographic dark energy with the Hubble horizon cutoff into an f(Q) gravity framework.

Method: Constrained model parameters using DESI BAO, P-BAO, and CC datasets via MCMC; analyzed H(z), q(0), z_tr, Om(z), ω_eff, and energy conditions; performed perturbation analysis for stability.

Result: The model's H(z) matches observations well (high R², low χ²/AIC/BIC), transitions to acceleration at z_tr~0.56-0.72, exhibits quintessence-like ω_eff, and satisfies WEC/DEC/NEC while violating SEC at low redshifts, showing dynamical stability.

Conclusion: The FHDE-inspired f(Q) gravity offers a stable, observationally consistent geometric dark energy alternative to ΛCDM within symmetric teleparallel gravity, successfully explaining cosmic acceleration.

Abstract: In order to explain the late-time acceleration of the Universe, we present a reconstructed version of the $f(Q)$ gravity theory in this work, which is inspired by the integrating the fractional holographic dark energy with the Hubble horizon as the infrared cutoff. This reconstructed $f(Q)$ gravity model shows a geometrically motivated dark energy component and naturally recovers General Relativity in the appropriate limit. The free parameters of the model are constrained using the latest DESI BAO data, previous BAO compilations (P-BAO), and cosmic chronometer (CC) datasets through a Markov Chain Monte Carlo (MCMC) analysis. The reconstructed Hubble parameter $H(z)$ exhibits excellent consistency with observational data, with high values of $R^2$ and low values of $χ^2_{\min}$, AIC, and BIC, confirming the model's strong statistical performance relative to $Λ$CDM. With current $q(0) \in [-0.40, -0.32]$ and a transition redshift $z_{\text{tr}} \sim 0.56$--$0.72$, the dynamical diagnostics show a smooth transition from a decelerated to an accelerated phase. While the $Om(z)$ diagnostic exhibits a negative slope, indicating that the model is not $Λ$CDM, the effective equation-of-state parameter $ω_{\text{eff}}(z)$ stays within the quintessence regime ($-1 < ω_{\text{eff}} < -1/3$). The analysis of classical energy conditions shows that the WEC, DEC, and NEC are satisfied throughout the cosmic evolution, with a violation of the SEC at lower-redshift, which is consistent with late-time acceleration. Linear homogeneous perturbation analysis further confirms the model's dynamical stability. Conclusively, the FHDE-inspired reconstructed $f(Q)$ gravity provides a stable, observationally compatible, and geometrically motivated alternative to $Λ$CDM, that successfully describes the late-time cosmic acceleration within the symmetric teleparallel framework.

</details>


### [15] [Shadow geometry of Kerr MOG naked singularity and analysis of accretion disk luminosity](https://arxiv.org/abs/2512.03896)
*Saira Yasmin,Mubasher Jamil*

Main category: gr-qc

TL;DR: The paper investigates the shadow properties and accretion disk radiation of Kerr Modified Gravity Naked Singularities (KMNSs), showing that their shadows can appear closed, open, or vanish based on parameters like spin, MOG parameter α, and observer angle, offering a way to distinguish KMNS from standard Kerr naked singularities.


<details>
  <summary>Details</summary>
Motivation: To understand the observable features of KMNSs and how modifications in gravity (via MOG's α parameter) and spin affect their shadows and accretion disk emissions, potentially providing a method to distinguish KMNS from other black hole or naked singularity models.

Method: Analysis of spacetime geometry and equations of motion for test particles around KMNSs. Exploration of shadow shapes under varying parameters (spin a, α, inclination angle). Modeling of accretion disk flux, temperature, and luminosity using simplified radiative models. Comparison of KMNS with standard Kerr naked singularities (α=0).

Result: KMNS shadows exhibit unique features like gaps not seen in black holes, with visibility dependent on a, α, and observer inclination. Accretion disk radiation properties (flux distribution, spectral luminosity) differ显著 with α and spin, offering observational signatures to distinguish KMNS from KNS.

Conclusion: The study highlights that modified gravity parameters and spin significantly alter the observable characteristics of naked singularities, suggesting that observational data on shadow morphology and accretion disk emissions could potentially identify KMNSs and differentiate them from Kerr black holes or standard naked singularities.

Abstract: Naked singularities are hypothetical astrophysical entities featuring gravitational singularities without event horizons. In this study, we analyze the shadow properties of Kerr Modified Gravity (Kerr MOG) naked singularities (KMNSs). We show that the KMNS shadow can appear closed, open, or even vanish, depending on the dimensionless spin parameter a, the modified gravity parameter alpha, and the observer's inclination angle. We identify the critical conditions under which the KMNS shadow develops a gap, a unique feature not present in BH shadows. We analyze the properties of a thin accretion disk surrounding a KMNS, within the framework of MOG characterized by the parameter alpha. The study includes a detailed examination of the spacetime geometry and the equations of motion for test particles. In addition, we adopt a simplified model for the disk's radiative flux, temperature distribution, and spectral luminosity. Our analysis primarily focuses on the flux distribution of the accretion disk around KMNS with identical mass but varying spin and MOG deformation parameters. This allows us to explore how modifications in rotation and the MOG parameter alpha influence the radiative properties of the disk. Further, these observational signatures may serve as effective tools for clearly distinguishing KMNS from standard Kerr naked singularities (KNSs), where the MOG parameter alpha = 0

</details>


### [16] [Primordial Gravitational Wave Birefringence in a de Sitter Background with Chern-Simons Coupling](https://arxiv.org/abs/2512.03919)
*Abhishek Rout,Brett Altschul*

Main category: gr-qc

TL;DR: This paper explores parity-violating effects in gravitational waves using Chern-Simons modified gravity, showing that tensor perturbations in a de Sitter universe exhibit chiral dispersion relations, birefringence, and helicity-dependent amplification linked to a scalar field background. These effects imprint permanent parity violations in the primordial tensor spectrum and connect to dark matter phenomenology.


<details>
  <summary>Details</summary>
Motivation: The motivation is twofold: first, to understand how Chern-Simons modifications affect gravitational wave propagation in an expanding universe, and second, to explore potential observational signatures linking modified gravity to dark matter through axion-like particle dynamics.

Method: The authors analyze transverse-traceless tensor perturbations in de Sitter spacetime under Chern-Simons gravity. They mode-decompose the scalar background field to derive wave equations, examine sub/super-horizon limits, and calculate dispersion relation modifications. Solutions are constructed to show helicity-dependent gravitational wave amplification and phase differences.

Result: Key results include chiral corrections causing birefringence with both amplitude and velocity differences coupling to scalar field phase. Inside the horizon, phase differences grow quadratically until freezing outside, leaving permanent parity violation marks. The Chern-Simons scalar's mass connects gravitational birefringence to axion-like dark matter behavior.

Conclusion: Chern-Simons modified gravity creates detectable parity violations in primordial gravitational waves, offering a testable link between modified gravity, cosmological inflation, and dark matter models through observable tensor spectrum asymmetries.

Abstract: In this work, we investigate tensor perturbations in a de Sitter background within the framework of Chern-Simons modified gravity. We introduce transverse-traceless perturbations and analyze how the Chern-Simons Cotton tensor induces parity-violating modifications to gravitational wave propagation, while the Pontryagin density vanishes at linear order. Using a mode decomposition of the scalar background field, we derive the sub- and super-horizon limits of the wave equations and uncover chiral corrections in the dispersion relations of tensor modes. The resulting birefringence exhibits both amplitude and velocity components, alternating with the phase of the scalar field. Particular solutions sourced by the scalar background show helicity-dependent amplification and a characteristic scaling of the radiated flux that reduces smoothly to the Minkowski limit. The accumulated phase difference between right- and left-handed modes grows quadratically inside the horizon and becomes frozen outside, leaving a permanent parity-violating imprint in the primordial tensor spectrum. Finally, by promoting the Chern-Simons field to a massive dark matter candidate, we demonstrate how its mass-dependent dynamics connect gravitational birefringence to axion-like dark matter phenomenology.

</details>


### [17] [Screening of dipolar emission in two-scale Gauss-Bonnet gravity](https://arxiv.org/abs/2512.04083)
*Farid Thaalba,Leonardo Gualtieri,Thomas P. Sotiriou,Enrico Trincherini*

Main category: gr-qc

TL;DR: This paper explores black holes in a modified gravity theory combining shift-symmetric scalar Gauss-Bonnet gravity with a cubic Galileon interaction, allowing for smaller black holes and a screening mechanism. This enables compatibility with gravitational-wave constraints while affecting key observables in the strong-field regime.


<details>
  <summary>Details</summary>
Motivation: To investigate how introducing a cubic Galileon interaction with a distinct energy scale modifies the phenomenology of shift-symmetric scalar Gauss-Bonnet gravity, particularly regarding black hole properties and consistency with observational constraints.

Method: The authors analyze the implications of incorporating a cubic Galileon term into the shift-symmetric scalar Gauss-Bonnet theory. They study black hole solutions, screening mechanisms near horizons, and effects on observables like scalar charge and innermost stable circular orbit parameters.

Result: The cubic interaction enables smaller stable black holes and a screening mechanism that allows large Gauss-Bonnet couplings to avoid violating gravitational-wave bounds. Observable quantities are most impacted for small black holes, maintaining a technically natural multi-scale effective field theory.

Conclusion: The theory provides new avenues to test gravity in strong-field regimes by introducing controllable hierarchies between interaction scales, offering a natural framework for exploring modified gravity without conflicting with existing observational limits.

Abstract: We study black holes in shift-symmetric scalar Gauss-Bonnet gravity extended by a cubic Galileon interaction with a distinct energy scale. Introducing this hierarchy profoundly modifies the theory's phenomenology. The cubic interaction allows for smaller black holes, and can generate a screening mechanism near the horizon, making large Gauss-Bonnet couplings consistent with gravitational-wave bounds. Observable quantities such as the scalar charge, the innermost stable circular orbit, and its frequency are most affected for small black holes. The resulting multi-scale effective field theory remains technically natural and offers new avenues to probe gravity in the strong-field regime.

</details>


<div id='astro-ph.HE'></div>

# astro-ph.HE [[Back]](#toc)

### [18] [Taming the Tarantula: How Stellar Wind Feedback Shapes Gas and Dust in 30 Doradus](https://arxiv.org/abs/2512.03129)
*Jennifer A. Rodriguez,Laura A. Lopez,Lachlan Lancaster,Anna L. Rosen,Omnarayani Nayak,Sebastian Lopez,Tyler Holland-Ashford,Trinity L. Webb*

Main category: astro-ph.HE

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: Observations of massive star-forming regions show that classical stellar wind models over-predict the luminosity of the X-ray emitting gas, indicating a significant fraction of wind energy is lost. In this paper, we present a multi-wavelength analysis of the giant HII region 30 Doradus and its central star cluster R136 using 2 Ms of Chandra X-ray Observatory data, combined with James Webb Space Telescope and Hubble Space Telescope imaging and Spitzer spectral-energy distributions, to investigate how the hot gas energy is lost through turbulent mixing, radiative cooling, and physical leakage. We compare the spatial and spectral properties of the hot gas with those of the warm ionized gas and dust. We find no significant correlation between the dust and hot gas temperatures, suggesting they are not directly coupled and that the dust resides in the swept-up shells where it is heated radiatively. H$α$ and X-ray surface brightness profiles show that the X-rays peak interior to the H$α$ shells, demonstrating partial confinement of the hot gas. The fragmented shell structure and bright X-ray interior that declines near the H$α$ shell reflect efficient cooling from turbulent mixing at the hot-cold interface. We compare against recent simulations of stellar-feedback driven bubbles which have broad agreement with the morphology of the X-ray and H$α$ emission, but the simulations produce a dip in the interior X-ray surface brightness and a lack of hard X-rays compared to the observations. These differences may suggest thermal conduction is important as mass-loading of the hot bubble could reproduce the X-ray observables.

</details>


### [19] [GRB 230204B: GIT Discovery of a Fast Fading Afterglow Associated with an Energetic GRB from a Massive-Star Progenitor](https://arxiv.org/abs/2512.03136)
*Vishwajeet Swain,Varun Bhalerao,Harsh Kumar,Mehul Goyal,Ankur Ghosh,Utkarsh Pathak,Poonam Chandra,Tomas Ahumada,G. C. Anupama,Suman Bala,Sudhanshu Barway,Joshua S. Bloom,Dimple Dimple,Viraj R. Karambelkar,Mansi M. Kasliwal,Kuntal Misra,Josiah Purdum,Divita Saraogi,Jesper Sollerman,Aswin Suresh,Stefan J. van der Walt,Gaurav Waratkar*

Main category: astro-ph.HE

TL;DR: A multi-wavelength analysis of GRB 230204B reveals strong thermal and non-thermal emission components, indicating an energetic jet expanding into a wind medium from a massive star progenitor. The study highlights correlations between prompt emission and afterglow emissions.


<details>
  <summary>Details</summary>
Motivation: To comprehensively study the prompt and afterglow emissions of GRB 230204B to better understand its energetics, emission mechanisms, and the environment around its progenitor star.

Method: The study utilized multi-wavelength observations of both prompt and afterglow emissions. Data from the GROWTH-India Telescope provided measurements of the afterglow's rapid fading. Afterglow modeling was performed to infer jet energy and circumstellar medium properties. The evolution of emission components was analyzed against theoretical models.

Result: The prompt emission showed thermal photospheric emission with a non-thermal high-energy component consistent with theory. Afterglow analysis indicated an $E_{tot} \gtrsim 10^{52} erg$ jet in a wind-type medium, supporting a massive star progenitor with strong winds. Correlations between prompt and afterglow emissions were explored.

Conclusion: The findings support progenitor models involving massive stars with strong winds and suggest viewing geometry close to the jet axis. The combined analysis strengthens the link between GRB prompt emission properties and their circumstellar environments.

Abstract: We present a comprehensive multi-wavelength study of a bright gamma-ray burst GRB 230204B, analyzing both prompt and afterglow emissions. This GRB is highly energetic, with an isotropic equivalent energy emission $E_{\mathrm{iso}} \sim 2.2 \times 10^{54}\ \mathrm{erg}$, released during the prompt emission. The GROWTH-India Telescope discovered a bright afterglow ($m_r = 15.55$) that faded rapidly ($\propto t^{-1.82}$). The prompt emission shows strong thermal photospheric emission, along with a non-thermal high-energy component. We explore the evolution of these components and find them to be consistent with theoretical expectations. Afterglow modeling reveals an energetic jet $E_{tot} \gtrsim 10^{52}\ \mathrm{erg}$ expanding into a wind-type medium viewed nearly on-axis, suggesting a massive star progenitor with strong winds. We also explore correlations between the prompt emission and afterglow that may help to understand the complete picture of GRB progenitors.

</details>


### [20] [A Wind-Driven Origin for the Firework Morphology of the Supernova Remnant Pa 30](https://arxiv.org/abs/2512.03140)
*Eric R. Coughlin,Greg Salvesen,Dheeraj R. Pasham*

Main category: astro-ph.HE

TL;DR: The paper explains the firework-like filament structure of Pa 30, a remnant of the 1181 AD Galactic type Iax supernova, through hydrodynamic simulations. The filaments arise from Rayleigh-Taylor instability at the wind-CSM interface but remain intact due to Kelvin-Helmholtz stability and density profile characteristics. Simulations align with observed filament properties and predict truncation when wind and CSM densities equalize.


<details>
  <summary>Details</summary>
Motivation: To understand the unusual radial filamentary morphology of Pa 30 and determine the physical mechanisms responsible for their formation and evolution, distinguishing between competing theories like wind termination shock vs. RTI/KHI instabilities.

Method: 2D hydrodynamical simulations modeled the interaction between the white dwarf's fast wind and the circumstellar medium. Calculated filament parameters (speed, density, temperature) and compared them to observational data. Analyzed timescales for Rayleigh-Taylor and Kelvin-Helmholtz instabilities.

Result: Simulations reproduce observed filament features: elongation, density contrast stability, and predicted truncation timeline (1-10 years post-SN). Confirms filaments form during active RTI phase, with KHI suppression due to slow density decline in wind. Central filament-free area explained by finite RTI operation time, not wind shocks.

Conclusion: Firework filaments in supernova remnants require high ejecta-CSM density contrast. The study provides a physical framework for interpreting similar structures in other astrophysical systems with comparable conditions, such as other SN Iax remnants or stellar winds interacting with CSM.

Abstract: Pa 30 -- the likely remnant of the Galactic type Iax supernova of 1181 AD -- displays an unusual, firework-like morphology, consisting of radial filaments extending from a common center, where a white dwarf (WD) currently drives a very fast wind (speed $\gtrsim 10^{4}$ km s$^{-1}$). We propose the filaments arose from the Rayleigh-Taylor-unstable nature of the interface between the circumstellar medium (CSM) and the shocked wind launched by the natal WD; the filaments then elongated intact due to the Kelvin-Helmholtz-stable nature of the large initial density contrast between the wind and CSM, supplemented by the slowly declining wind density profile (relative to homologously expanding ejecta). To support this interpretation, we present two-dimensional hydrodynamical simulations and derive the filament properties, including their speed, density, and temperature, all of which are consistent with observations. We suggest the filaments elongate until the wind and CSM densities become comparable at the contact discontinuity, which occurs within 1--10 years, and then truncate because the RTI halts. The subsequent KHI growth timescale across the current width of the filaments is longer than the age of Pa 30, so they remain intact. The filament-less central region in Pa 30 is therefore more likely a consequence of the finite timescale over which the RTI operates, rather than a wind termination shock. In general, firework-like filaments may form in other systems, provided there is a sufficiently large density contrast between the ejecta and its surroundings.

</details>


### [21] [Magnetic Field Amplification and Particle Acceleration in Weakly Magnetized Trans-relativistic Electron-ion Shocks](https://arxiv.org/abs/2512.03169)
*Taiki Jikei,Daniel Groselj,Lorenzo Sironi*

Main category: astro-ph.HE

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: We investigate the physics of quasi-parallel trans-relativistic shocks propagating in weakly magnetized plasmas by means of long-duration two-dimensional particle-in-cell simulations. The structure of the shock precursor is shaped by a competition between the Bell instability and the Weibel instability. The Bell instability is dominant at relatively high magnetizations $(σ\gtrsim10^{-3})$, whereas the Weibel instability prevails at lower magnetizations $(σ\lesssim10^{-4})$. Bell-dominated shocks efficiently accelerate ions, converting a fraction $\varepsilon_{\mathrm{i}}\sim0.2$ of the upstream flow energy into downstream nonthermal ion energy. The maximum energy of nonthermal ions exhibits a Bohm scaling in time, as $E_{\max}\propto t$. A much smaller fraction $\varepsilon_{\mathrm{e}}\ll0.1$ of the upstream flow energy goes into downstream nonthermal electrons in the Bell-dominated regime. On the other hand, Weibel-dominated shocks efficiently generate both nonthermal ions and electrons with $\varepsilon_{\mathrm{i}}\sim\varepsilon_{\mathrm{e}}\sim0.1$, albeit with a slower scaling for the maximum energy, $E_{\mathrm{max}}\propto t^{1/2}$. Our results are applicable to a wide range of trans-relativistic shocks, including the termination shocks of extragalactic jets, the late stages of gamma-ray burst afterglows, and shocks in fast blue optical transients.

</details>


### [22] [Voyager 1 Data Reveals Signatures of the Local Gas and Cosmic-Ray Source Distributions](https://arxiv.org/abs/2512.03385)
*Troy A. Porter,Igor V. Moskalenko,Alan C. Cummings,Guðlaugur Jóhannesson*

Main category: astro-ph.HE

TL;DR: The study examines how the nearby interstellar medium (ISM) influences local cosmic-ray (CR) spectra using the GALPROP code. It finds that low-energy CR sources are within ~150-200 pc and that interstellar gas distributions affect spectral features, supporting the presence of primary Boron in CR data. This highlights the need for detailed local ISM modeling to resolve discrepancies between CR measurements and global propagation models.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of the local interstellar environment on measured cosmic-ray spectra, address inconsistencies between observational data and global models, and determine the proximity of CR sources to the Solar System.

Method: Utilizes the GALPROP code to model cosmic-ray propagation, incorporating variations in local gas and source distributions. Compares simulations with Voyager 1's interstellar data to assess the influence of ISM structure on CR spectra.

Result: Identifies that the nearest CR source is 150-200 pc away, confirms primary Boron in low-energy spectra (supporting Cummings et al., 2025), and shows that ISM structure considerations improve alignment between local CR data and global propagation models.

Conclusion: Accurate inference of cosmic-ray propagation parameters requires detailed modeling of the immediate Galactic environment. Accounting for nearby ISM structures reduces tensions between local measurements and global models, emphasizing the importance of local environmental factors in CR studies.

Abstract: We investigate the effects of the nearby interstellar medium (ISM) on the locally measured cosmic-ray (CR) spectra. Using the GALPROP code we explore how variations in the local gas and source distributions affect spectral features at low energies. Comparing with recent Voyager 1 data taken in the local ISM, we show that for a realistic interstellar gas distribution, the nearest source of the low energy CR particles observed nearby the Solar system is constrained to be within the range ~150-200 pc distant. We find that the modelling supports the conclusion of Cummings et al. (2025) that there is a significant fraction of primary Boron in its observed spectrum at low energies. Our study shows that detailed modelling of the immediate Galactic environment is required to robustly infer Galactic CR propagation parameters from local measurements, and that accounting for nearby ISM structure can alleviate tensions between direct CR data and global propagation models.

</details>


### [23] [A Deep Chandra X-ray Survey of a Luminous Quasar Sample at $z\sim$ 7](https://arxiv.org/abs/2512.03433)
*Xiangyu Jin,Feige Wang,Jinyi Yang,Xiaohui Fan,Fuyan Bian,Jiang-Tao Li,Weizhe Liu,Yichen Liu,Jianwei Lyu,Maria Pudoka,Wei Leong Tee,Yunjing Wu,Haowen Zhang,Yongda Zhu*

Main category: astro-ph.HE

TL;DR: New Chandra observations of luminous quasars at z>6.5 reveal correlations between X-ray properties (L_X, α_OX) and UV luminosity, with potential links to fast disk winds and high accretion rates. No strong correlation found between quasar and star formation luminosities.


<details>
  <summary>Details</summary>
Motivation: To study high-redshift quasars' X-ray emission and correlations with other properties to understand their accretion processes and environment.

Method: Chandra X-ray observations combined with ground-based NIR and ALMA data. Analyzed correlations between X-ray luminosity, α_OX, UV luminosity, C IV blueshift, and IR luminosity.

Result: Most z>6.5 quasars follow α_OX-L_uv relation but show scatter. Potential α_OX-C IV blueshift correlation suggests soft SED linked to fast winds. Photon index Γ=2.41±0.27 from high accretion rates. No strong correlations with IR luminosity indicate weak quasar-star formation connection.

Conclusion: High accretion rates dominate z>6.5 quasar X-ray properties. X-ray-UV relations and wind signatures show active accretion disk dynamics. Lack of IR correlations implies quasars' luminosity isn't tightly coupled with galaxy star formation at this epoch.

Abstract: We present new Chandra observations of seven luminous quasars at $z>6.5$. Combined with archival Chandra observations of all other known quasars, they form nearly complete X-ray observations of all currently known $z\sim7$ quasars with $M_{1450}<-26.5$, except for J0313$-$1806 at $z=7.642$ and J0910$-$0414 at $z=6.636$. Together with existing ground-based NIR spectroscopy and ALMA observations, we investigate the correlations between X-ray emission (the X-ray luminosity $L_{\rm X}$ and the optical/UV-to-X-ray spectral slope $α_{\rm OX}$) and various quasar properties (rest-UV luminosity $L_{\mathrm{2500\ \mathring{A}}}$, bolometric luminosity $L_{\rm bol}$, C IV blueshift, and infrared luminosity $L_{\rm IR}$). We find most $z>6.5$ quasars follow a similar $α_{\rm OX}-L_{\mathrm{2500\ \mathring{A}}}$ relation as $z\sim1-6$ quasars, but also display a large scatter. We find a potential correlation between $α_{\rm OX}$ and the C IV blueshift, suggesting a soft optical/UV-to-X-ray SED shape is frequently associated with fast disk winds. Furthermore, we analyze the X-ray spectrum of 11 quasars at $z>6.5$ with Chandra detection, and find the best-fit photon index $Γ$ is $2.41\pm0.27$, which is likely driven by high accretion rates of $z>6.5$ quasars. In addition, we find there are no significant correlations between either $L_{\rm X}$ and $L_{\rm IR}$, nor $L_{\rm bol}$ and $L_{\rm IR}$, suggesting no strong correlations between quasar luminosity and star formation luminosity for the most luminous quasars at $z>6.5$.

</details>


### [24] [Subgrid Mean-field Dynamo Model with Dynamical Quenching in General Relativistic Magnetohydrodynamic Simulations](https://arxiv.org/abs/2512.03443)
*Hongzhe Zhou,Yosuke Mizuno,Zhenyu Zhu*

Main category: astro-ph.HE

TL;DR: The paper presents a subgrid model for helical large-scale dynamos in general-relativistic simulations of accretion disks, using a viscosity parameter α_SS to reproduce previous dynamo behavior and study outflows and field configurations under varying black hole spin and plasma conditions.


<details>
  <summary>Details</summary>
Motivation: To enable self-consistent simulations of accretion disks that include intrinsic dynamo processes, as prior studies show initial magnetic field conditions significantly impact disk evolution. The lack of a suitable subgrid model for thin disks motivated developing this framework.

Method: A subgrid model incorporating dynamical quenching was developed by combining past numerical and analytical results on helical dynamos. The model uses α_SS as the sole input parameter. Implemented in general-relativistic resistive MHD simulations, tested against local/global butterfly diagrams. Parameter studies were conducted varying α_SS, black hole spin (a_BH), and plasma β.

Result: The model successfully replicates butterfly diagrams with appropriate α_SS. With aggressive parameters (α_SS=0.02, a_BH=0.9375), weak polar outflows (Lorentz factor ~1.2) are generated. Negative a_BH leads to Newtonian-like fields, while positive a_BH causes distorted field loops and sporadic dynamo cycles. Dynamical α_SS determination based on plasma β achieves self-consistent field amplification.

Conclusion: The subgrid model provides a viable framework for incorporating dynamo action in relativistic disk simulations. Future work should explore effects of magnetization and extend to more physical conditions. The model's ability to avoid prescribing α_SS marks progress toward fully self-consistent treatments.

Abstract: Large-scale magnetic fields are relevant for a number of dynamical processes in accretion disks, including driving turbulence, reconnection events, and launching outflows. Numerical simulations have indicated that the initial strengths and configurations of the large-scale magnetic fields have a direct imprint on the outcome of an accretion disk evolution. To facilitate future self-consistent simulations that include intrinsic dynamo processes, we derive and implement a subgrid model of a helical large-scale dynamo with dynamical quenching in general-relativistic resistive magnetohydrodynamical simulations of geometrically thin accretion disks. By incorporating previous numerical and analytical results of helical dynamos, our model features only one input parameter, the viscosity parameter $α_\text{SS}$. We demonstrate that our model can reproduce butterfly diagrams seen in previous local and global simulations. With rather aggressive parameter choice of $α_\text{SS}=0.02$ and black hole spin $a_\text{BH}=0.9375$, our thin-disk model launches weak collimated polar outflows with Lorentz factor $\simeq 1.2$, but no polar outflow is present with less vigorous turbulence or less positive $a_\text{BH}$. With negative $a_\text{BH}$, we find the field configurations to appear more similar to Newtonian cases, whereas for positive $a_\text{BH}$, the poloidal field loops become distorted and the cycle period becomes sporadic or even disappears. Moreover, we demonstrate how $α_\text{SS}$ can avoid to be prescribed and instead be determined by the local plasma beta. Such a fully dynamical subgrid dynamo allows for self-consistent amplification of the large-scale magnetic fields.

</details>


### [25] [Neutrino-dominated relativistic shocked accretion flow around rotating black hole: implications for short gamma-ray bursts](https://arxiv.org/abs/2512.03447)
*Amit Kumar,Sayan Chakrabarti,Santabrata Das*

Main category: astro-ph.HE

TL;DR: This paper explores how neutrino-dominated accretion flows (NDAFs) around stellar-mass black holes can power gamma-ray bursts (GRBs), showing that shocked NDAF solutions explain diverse GRB energy outputs. The study links black hole parameters (mass, spin) and disk mass to GRB luminosities, finding that smaller low-spin black holes match short GRB luminosities, while massive high-spin ones achieve similar brightness. A key correlation emerges where disk mass decreases with higher spin, independent of black hole mass.


<details>
  <summary>Details</summary>
Motivation: To understand the physical mechanisms behind GRB energetics and diversity, particularly through modeling the central engine as a NDAF around a black hole formed in compact binary mergers.

Method: Solved hydrodynamic equations to generate transonic NDAF solutions with shock transitions. Explored parameters like black hole mass, spin, and accretion rate. Used numerical simulations of neutron star and black hole-neutron star mergers to estimate remnant black hole properties and post-merger disk masses.

Result: Shocked NDAFs can naturally explain GRB energy diversity. Short GRBs' luminosities are replicable with low-mass, low-spin black holes or high-mass, high-spin counterparts. A robust inverse correlation between disk mass and black hole spin was found, independent of black hole mass.

Conclusion: NDAF models provide a unified framework for GRB energetics, linking observational diversity to black hole parameters. The spin-disk mass relationship offers constraints on merger outcomes and central engine behavior in GRB progenitors.

Abstract: We investigate the physical properties of the central engine powering gamma-ray bursts (GRBs), modelled as a stellar-mass black hole accreting via a neutrino-dominated accretion flow (NDAF). By solving the governing hydrodynamic equations, we obtain global transonic NDAF solutions featuring shock transitions and examine their role in powering GRB energetics. The NDAF solutions are explored over a broad range of black hole parameters, including its mass ($M_{\rm BH}$) and spin ($a_{\rm k}$), and accretion rate ($\dot{M}$). We find that shocked NDAFs can naturally account for the observed diversity in GRB energy output. Incorporating results from numerical simulations of binary neutron star and black hole-neutron star mergers, we estimate the remnant black hole mass and spin parameters for the predicted range of post-merger disk mass ($M_{\rm disk}$). Our analysis reveals that small-mass black holes with relatively low spin values can adequately reproduce the luminosities of short GRBs (SGRBs), whereas identical GRB luminosities can also be achieved for more massive black holes possessing higher spin values. Finally, we uncover a robust correlation between the black hole spin and disk mass such that $M_{\rm disk}$ decreases with increasing $a_{\rm k}$, remaining largely independent of the black hole mass ($M_{\rm BH}$) powering GRBs.

</details>


### [26] [The helium common-envelope wind scenario for SN 2020eyj](https://arxiv.org/abs/2512.03531)
*Xiang-Cun Meng,Philipp Podsiadlowski*

Main category: astro-ph.HE

TL;DR: SN 2020eyj is the first Type Ia supernova with a compact helium-rich CSM, explained by a helium common-envelope wind model.


<details>
  <summary>Details</summary>
Motivation: To address the mystery of how a massive helium-rich CSM forms around a Type Ia supernova in a single-degenerate scenario.

Method: Proposed the HeCEW model where a CE forms if initial WD mass > 1.1 Msun and helium star > 1.8 Msun, leading to CE mass > 0.3 Msun. Simulated CE dynamics and compared parameters with SN 2020eyj observations.

Result: Model successfully explains SN 2020eyj's properties: massive He-rich CSM, dim brightness, low ejecta velocity, and low birth rate.

Conclusion: HeCEW model provides a viable formation mechanism for Type Ia SNe with helium-rich CSM, expanding our understanding of binary evolution pathways.

Abstract: SN 2020eyj is the first type Ia supernova (SN Ia) showing the signature of a compact helium-rich circumstellar material (CSM). Such a large CSM is difficult to explain in a single-degenerate scenario where the donor star is a helium star. Here we show that, under certain conditions, it is possible that the transfer of helium leads to a common envelope (CE) engulfing the system, similar to the common-envelope wind model proposed by Meng \& Podsiadlowski (2017). If in such a helium common-envelope wind (HeCEW) model the initial white dwarf (WD) mass is larger than 1.1 $M_{\rm \odot}$ and the helium star more massive than 1.8 $M_{\rm \odot}$, the mass of a helium CE can be larger than 0.3 $M_{\rm \odot}$ prior to supernova explosion. The CE mass heavily depends on the initial parameters of the binary system. A dynamical CE ejection event could occur shortly before the supernova, and then our model may naturally explain the properties of SN 2020eyj, specifically the massive He-rich CSM, its dim peak brightness, low ejecta velocity and low birth rate.

</details>


### [27] [Resolving the Multiple Component Outflows in PG 1211+143: I. The Fe-K Absorption Structure and UFO Forest](https://arxiv.org/abs/2512.03533)
*Misaki Mizumoto,James N. Reeves,Valentina Braito,Ehud Behar,Chris Done,Kouichi Hagino,Steven B. Kraemer,Gabriele A. Matzeu,Hirofumi Noda,Mariko Nomura,Shoji Ogawa,Ken Ohsuga,Atsushi Tanimoto,Tracey J. Turner,Yoshihiro Ueda,Satoshi Yamada,Sreeparna Ganguly,Paolo Somenzi*

Main category: astro-ph.HE

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: We present the initial high-resolution X-ray spectroscopic observations of the Fe-K absorption structure in the luminous nearby quasar PG 1211+143, utilizing the X-ray Imaging and Spectroscopy Mission (XRISM). The primary objective is to characterize the Fe-K absorption features due to Ultra-Fast Outflow (UFO) in this Eddington-luminosity source. Observations were conducted with XRISM's Resolve and Xtend instruments, complemented by simultaneous data from XMM-Newton and NuSTAR. A historically bright phase was captured. The Resolve spectra clearly reveal a prominent P Cygni profile and resolves the Fe-K absorption into six distinct velocity components, ranging from $v = -0.074c$ to $-0.405c$. A similar superposition of multiple UFOs has been reported in PDS~456, suggesting that such a ``UFO forest'' structure may be a common feature of near Eddington-luminosity sources. Some UFO components exhibit narrow line widths of approximately $σ\sim 200\,\mathrm{km\,s^{-1}}$, which may indicate that the outflows have reached their terminal velocities, thereby resulting in a smaller velocity shear. The mass outflow rate is estimated to be $\dot{M}_\mathrm{out} \sim 1~M_{\odot}~\text{yr}^{-1}$, which is of the order of the Eddington accretion rate. This suggests a physically plausible scenario where the outflow is a significant channel for mass ejection.

</details>


### [28] [Statistical and Temporal Analysis of Multi-component Burst-clusters from the Repeating FRB 20190520B](https://arxiv.org/abs/2512.03591)
*Jia-heng Zhang,Chen-Hui Niu,Yu-hao Zhu,Di Li,Wei-yang Wang,Yi Feng,Xin-ming Li,Jia-rui Niu,Pei Wang,Yun-wei Yu,Yong-kun Zhang,Xiao-ping Zheng*

Main category: astro-ph.HE

TL;DR: The study analyzes the multi-component bursts of FRB 20190520B and four other FRBs observed by FAST, revealing scale-free behavior in component counts consistent with SOC, and no periodicity in temporal properties.


<details>
  <summary>Details</summary>
Motivation: To understand FRB emission mechanisms and progenitor systems by examining multi-component burst structures and their implications for origin theories.

Method: Systematic analysis of burst morphology for FRB 20190520B and comparison of component distributions across four active FRBs using FAST observations.

Result: Multi-component bursts show spectral similarity to single-peak bursts, lack detected periodicity, and follow a power-law distribution indicating SOC processes. They constitute 12-30% of bursts across sources.

Conclusion: Findings suggest SOC as a key process in FRB emission, highlight burst variability, and offer clues about progenitor environments and emission physics.

Abstract: Fast Radio Bursts (FRBs) are bright, millisecond-duration extragalactic radio transients that probe extreme astrophysical environments. Many FRBs exhibit multi-component structures, which encode information about their emission mechanisms or progenitor systems and thus provide important clues to their origins. In this work, we systematically analyze the burst morphology of FRB 20190520B and compare component distributions across four active FRBs observed with FAST: FRB 20121102A, FRB 20190520B, FRB 20201124A, and FRB 20240114A. We find that multi-component burst-clusters show spectral properties similar to single-peak bursts, and no periodicity is detected in their temporal behavior. The component-count distributions follow a power law, revealing scale-free behavior consistent with self-organized criticality (SOC) processes. Multi-component clusters account for 12-30% of all detected bursts, regardless of source activity, providing new insights into burst-to-burst variability and the physical processes driving FRB emission.

</details>


### [29] [Trapped fireshell (halo) of photons and pairs around black-hole horizon: source for ultra-high-energy particles](https://arxiv.org/abs/2512.03702)
*She-Sheng Xue*

Main category: astro-ph.HE

TL;DR: The paper investigates the Compton-rocket effect in an opaque fireball with ultra-high-temperature photons and electron-positron pairs, leading to charged-particle acceleration and avalanches. These processes generate UHE electrons/protons that produce VHE photons/neutrinos. The findings are applied to Gamma-Ray Burst fireballs, proposing a metastable halo around black holes that emits UHE particles, which are analyzed for time-variation, observability, and simulation links.


<details>
  <summary>Details</summary>
Motivation: Understanding high-energy particle production mechanisms in extreme astrophysical environments like Gamma-Ray Bursts' central engines requires modeling photon-electron interactions. The study aims to explore how Compton-rocket effects and electron avalanches can generate observable VHE signatures.

Method: Theoretical analysis combining relativistic plasma physics and astrophysical modeling. Calculations of particle acceleration via multi-photon Compton interactions and avalanche processes within fireball dynamics, followed by simulating UHE particle luminosities over time and connecting outcomes to observational predictions.

Result: Non-trivial probabilities of UHE electrons/protons generation, time-evolving UHE particle luminosities, and predicted VHE photon/neutrino signatures linked to observable phenomena. Metastable fireball halo's role in particle emission is quantified.

Conclusion: The proposed mechanism explains potential origins of ultra-high-energy particles in Gamma-Ray Bursts, offering testable predictions for multi-messenger astrophysics through neutrino and VHE photon observations. Future simulations and telescopic data can validate this fireball halo model.

Abstract: We study the Compton-rocket effect of multi-photon interacting with electrons in an opaque fireball (or fire spot) of photons and $e^-e^+$ pairs at temperature $T_γ\gg m_e$. We find the charged-particle acceleration and the avalanche runaway process, leading to a non-trivial probability of ultra-high-energy (UHE) electrons and protons, which subsequently produce very-high-energy (VHE) photons and neutrinos. We show such peculiar dynamics using the Gamma-Ray Burst central engine fireball, whose inner part inflows and forms a gravitationally trapped fireshell (halo) around a black hole. The halo is a metastable, cooling via UHE particle emissions and blackbody radiation. We calculate the UHE particle luminosity varying in time, and discuss the peculiar features of such produced UHE particles, which lead to VHE particles, in connection with possible numerical simulations, observations and experiments.

</details>


### [30] [The broad-lined Type Ic supernova 2020lao reveals an energetic explosion with no central-engine signatures](https://arxiv.org/abs/2512.03716)
*M. D. Stritzinger,T. J. Moriya,S. Bose,P. A. Mazzali,P. Lundqvist,E. Karamehmetoglu,L. S. Arndt,C. Ashall,L. Galbany,W. B. Hoogendam,E. Baron,J. M. DerKacy,N. Elias-Rosa,E. Y. Hsiao,P. Höflich,E. Pian,E. A. M. Jensen,S. Moran,A. Pastorello,M. Shahbandeh,G. Valerin*

Main category: astro-ph.HE

TL;DR: Early observations of the Type Ic-BL supernova SN 2020lao reveal rapid optical rise, high kinetic energy, and no early optical/X-ray/radio afterglow, suggesting a compact progenitor and a choked-off jet mechanism.


<details>
  <summary>Details</summary>
Motivation: To understand the nature of SN 2020lao by analyzing its early-time light curves and spectra, assessing its progenitor star properties, energy budget, and the role of potential jets or engines in its explosion mechanism.

Method: Optical spectroscopy (from 48+ hours post-explosion), light curve analysis using TESS and ZTF data, Arnett modeling for bolometric light curves, spectral-synthesis modeling, and X-ray/radio non-detection constraints from VLA/Swift/XRT.

Result: SN 2020lao has a fast 8.8-day light curve rise, 0.2 Msun ⁵⁶Ni, 3.2 Msun ejecta, and high specific kinetic energy (7×10^51 erg/Msun). No early optical/X-ray/radio emission rules out relativistic ejecta and dense circumstellar material, suggesting a choked jet or non-relativistic explosion scenario.

Conclusion: SN 2020lao represents an extreme non-relativistic Type Ic-BL supernova with high kinetic energy but no detectable engine-driven activity, implying either a failed jet or off-axis viewing angle without significant circumstellar interaction.

Abstract: We present early-time observations of the broad-lined Type Ic supernova SN 2020lao, including optical spectroscopy beginning about 48 hr after the inferred explosion and extending to about 100 days. The explosion epoch is constrained with power-law fits to the rising TESS and ZTF light curves, with the first ZTF detection only about 27 hr after explosion. The optical light curves show a rapid 8.8 day rise and a peak luminosity typical of SNe Ic-BL (Mr=-18.5 mag). Unlike some engine-driven events, SN 2020lao shows no early optical excess or afterglow. The lack of a shock-cooling signature in the TESS/ZTF data constrains the progenitor to a compact Wolf Rayet-like star with radius of only a few solar radii. The spectra resemble those of the X-ray flash SN 2006aj but with higher velocities. Arnett modeling of the bolometric light curve, combined with Fe II velocities, yields a nickel mass of about 0.2 solar masses, an ejecta mass of about 3.2 solar masses, and a kinetic energy of about 23x10^51 erg, corresponding to a high specific kinetic energy of about 7x10^51 erg per solar mass. Spectral-synthesis modeling broadly reproduces the photospheric spectra and suggests a somewhat lower but still large specific kinetic energy (about 5 x 10^51 erg per solar mass). Although SN 2020lao and SN 2006aj produced similar 56Ni masses, SN 2020lao exhibits specific kinetic energies larger by a factor of several. Published VLA and Swift/XRT non-detections show no radio or X-ray afterglow, placing strong limits on relativistic ejecta and dense circumstellar material. Given its high specific kinetic energy, the absence of early excess emission and the radio/X-ray non-detections imply that any jet was either far off-axis or choked; otherwise, SN 2020lao represents an extreme non-relativistic SN Ic-BL.

</details>


### [31] [The anisotropy and magnetic field structure of neutron stars through gravitational wave](https://arxiv.org/abs/2512.03888)
*Zhao-Wei Du,Xi-Long Fan*

Main category: astro-ph.HE

TL;DR: The paper explores how gravitational wave (GW) signals from neutron star binaries can reveal their internal structure, including pressure anisotropy and magnetic fields. By introducing two magnetic field configurations (radial and transverse), the study shows that these factors increase maximum mass and alter tidal deformability, detectable with GW observations at certain signal quality levels.


<details>
  <summary>Details</summary>
Motivation: To understand the internal physics of neutron stars by extending the Tolman-Oppenheimer-Volkoff framework to incorporate pressure anisotropy and magnetic fields, which are crucial for interpreting gravitational wave data from neutron star mergers.

Method: The authors analyzed two magnetic field configurations (radial orientation dominated (RO) and transverse orientation dominated (TO)) with specific strength and decay models. They computed effects on neutron star maximum mass and tidal deformability Λ for equal mass binaries (1.2M⊙-1.2M⊙), then assessed detectability via gravitational wave signals using O4 power spectral density sensitivity.

Result: Both anisotropy and magnetic fields increase neutron star maximum mass and modify tidal deformability. RO fields produce more compact stars and larger Λ shifts detectable at SNR ~18, while TO fields require much higher SNR for detection. This indicates measurable GW signatures of internal structure.

Conclusion: Gravitational wave observations can effectively probe neutron star internal properties like pressure anisotropy and magnetic fields through detectable changes in tidal deformability and mass, offering insights into extreme matter physics.

Abstract: We investigate how gravitational wave (GW) observations can probe the internal physics of neutron stars by extending the Tolman-Oppenheimer-Volkoff framework to include pressure anisotropy and internal magnetic fields. Two representative magnetic field configurations, radial orientation dominated (RO) and transverse orientation dominated (TO), are implemented with strength and decay prescriptions. We found that both anisotropy and magnetic fields increase the maximum supported mass and modify the tidal deformability $Λ$, thereby imprinting measurable signatures on GW signals. For the equal mass binary ($1.2M_\odot$-$1.2M_\odot$), anisotropy neutron star with RO magnetic field yield more compact stars and a larger shift in $Λ$, allowing discrimination at signal-to-noise ratios (SNRs) as low as $\sim18$ using the O4 power spectra density. TO fields produce weaker effects and require substantially higher SNRs for detection. In conclusion, we conclude that gravitational waves are capable of probing the internal structure of neutron stars.

</details>


### [32] [The first 10years of the HAWC Gamma-Ray Observatory: science results](https://arxiv.org/abs/2512.03938)
*Anna Lia Longinotti,HAWC Collaboration*

Main category: astro-ph.HE

TL;DR: The HAWC Observatory's first decade of operation has produced over 90 publications, advancing understanding of high-energy astrophysics and particle physics through studies of TeV phenomena.


<details>
  <summary>Details</summary>
Motivation: To summarize HAWC's key scientific contributions over ten years, highlighting its role in exploring uncharted and poorly understood phenomena in the TeV energy range.

Method: The analysis uses data from HAWC's operational period (since March 2015) to review its publications and scientific outcomes, providing an overview of its contributions across various research areas.

Result: HAWC has enabled significant advancements in high-energy astrophysics and particle physics, producing over 90 peer-reviewed papers and clarifying previously unknown or poorly studied TeV-scale phenomena.

Conclusion: HAWC's decade-long observations have been pivotal in expanding knowledge of high-energy cosmic processes, establishing it as a cornerstone facility forTeV-scale research.

Abstract: The High-Altitude Water Cherenkov (HAWC) Observatory, located on the slopes of the Sierra Negra volcano in Mexico, began operations in March 2015. Over the past decade, HAWC has enabled the exploration of a broad range of topics in high-energy astrophysics and particle physics, resulting in more than 90 peer-reviewed publications. These studies have significantly advanced our understanding of several previously unexplored and poorly understood phenomena in the TeV energy regime. The present work provides an overview of the key scientific contributions of HAWC during its first ten years of operation.

</details>


### [33] [Line Polarization of Si II $λ$6355 Å in Type Ia Supernovae: A New Statistical Approach to Probe the Explosion Physics and Diversity](https://arxiv.org/abs/2512.03957)
*Aleksandar Cikota,Peter Hoeflich,Dietrich Baade,Ferdinando Patat,Lifan Wang,J. Craig Wheeler,Yi Yang,Elham Fereidouni,Divya Mishra*

Main category: astro-ph.HE

TL;DR: Spectropolarimetry of 24 Type Ia supernovae (SNe Ia) reveals asphericity classes linked to explosion mechanisms. Class I (off-center detonations, WD collisions) explains high polarization and velocity in transitional/faint SNe. Class II (rotating mergers) matches subluminous SNe with lower Si II polarization but stronger continuum polarization. These findings suggest viewing-angle effects may impact cosmological use of SNe Ia.


<details>
  <summary>Details</summary>
Motivation: To explore how spectropolarimetric data links SN ejecta structure to explosion mechanisms, and assess potential cosmological implications of their asphericity-induced luminosity variations.

Method: Analysis of Si II λ6355 Å polarization in 24 SNe Ia combined with multi-D radiative transfer simulations. Modeled two asphericity classes and compared observed velocity-polarization trends with simulations considering viewing angles.

Result: Class I scenarios explain high polarization in transitional/faint SNe; Class II fits subluminous cases. Statistical separation of classes shows diverse mechanisms at play. Viewing angles contribute to luminosity variations affecting cosmological analyses.

Conclusion: SN Ia asphericities are diverse, with distinct physical origins. Viewing-angle effects on luminosity may introduce biases in cosmological studies using SNe Ia, necessitating better angular distribution constraints.

Abstract: Spectropolarimetry provides a unique probe of ejecta asphericities, offering direct insights into the underlying explosion physics of Type Ia supernovae (SNe Ia). We analyze the statistical properties of pre-maximum spectropolarimetric data for 24 SNe Ia observed with VLT/FORS, focusing on the Si II $λ$6355 Åline. Previous studies have revealed a correlation between the peak Si II polarization degree and the expansion velocity. Here, we combine these observations with multi-dimensional non-LTE radiative transfer simulations. We consider two asphericity classes: (i) lopsided abundance distributions produced by off-center delayed-detonation transitions in near-$M_{Ch}$ white dwarfs or, for example, WD collisions (Class I), and (ii) global, axisymmetric density asphericities such as those arising from explosions of rapidly rotating WDs or mergers (Class II). Our model grid spans normal to subluminous SNe Ia and successfully reproduces the observed Si II velocity-polarization trend, with higher velocities associated with stronger asphericities. Consistent with observations, transitional SNe Ia and the faint end of the normal SNe Ia population show the highest Si II polarization and are best explained by Class I scenarios. In contrast, subluminous SNe Ia are dominated by Class II asphericities, characterized by lower Si II polarization but significant continuum polarization. The observed distribution of Si II polarization depends on both the observer's viewing angle $θ$ and the intrinsic asphericity. Statistical analysis of these spectropolarimetric snapshots enables the separation of Class I and Class II contributions and highlights the intrinsic diversity among SNe Ia. Our results imply viewing-angle-dependent luminosities in our local sample, which may have implications when using high-redshift SNe Ia as evidence for the need of non-standard cosmology.

</details>


### [34] [Hydrogen-Rich to Stripped-Envelope:Observational Continuity and Biases in CCSNe](https://arxiv.org/abs/2512.04010)
*Anjasha Gangopadhyay,Priscila J. Pessi*

Main category: astro-ph.HE

TL;DR: The review proposes that core-collapse supernovae (CCSNe) should be understood as a continuum rather than discrete subclasses, influenced by continuous variations in progenitor structure, mass-loss, and environment. CCSNe categories like Type II and stripped-envelope (IIb-Ib-Ic) overlap due to processes like stellar winds and binary interactions. Observational evidence supports gradual transitions between classes, emphasizing the need for advanced models and large datasets to map progenitor pathways to explosion diversity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of traditional discrete CCSN classifications by exploring continuums between subclasses, supported by observational evidence of overlaps and gradations. This shifts the perspective from strict boundaries to a multidimensional continuum, requiring analysis of physical processes to understand the true relationships between explosion outcomes.

Method: The method involves reviewing proposed continuums linking hydrogen-rich Type II SNe to stripped-envelope explosions and beyond. The authors assess physical processes (stellar winds, binary interactions, etc.) that enable transitions between subclasses and evaluate observational data to confirm or contest smooth transitions. This analytical approach synthesizes existing research to argue for a continuum model.

Result: The review finds evidence for CCSNe being part of a multidimensional continuum where conventional subclasses are reference points rather than barriers. Observations align with continuous variation caused by progenitor variables. Challenges remain in distinguishing evolutionary sequences from observational overlaps without further data and modeling.

Conclusion: The conclusion advocates redefining CCSN classification as a continuum to better capture the continuous nature of explosion diversity. Future work must combine large, homogenous datasets with advanced modeling to link progenitor histories to observed phenomena, ultimately validating the proposed multidimensional framework.

Abstract: Although historically classified into discrete subclasses, there is growing evidence that indicates that core-collapse supernovae (CCSNe) categories often overlap, reflecting continuous variations in progenitor structure, mass-loss history, and circumstellar environments rather than strictly distinct channels. In this review, we explore the proposed continua that link hydrogen-rich Type II SNe to stripped-envelope explosions (IIb-Ib-Ic), and that extend further into interaction-dominated and superluminous events. We discuss the physical processes-stellar winds, binary interaction, eruptive outbursts, and circumstellar interaction-that may produce graded outcomes across classes, while highlighting where observational evidence supports or challenges smooth transitions. We propose that CCSNe are better viewed as a multidimensional continuum of explosion outcomes, where traditional subclasses act as reference points rather than strict boundaries. Future progress will rely on large, homogeneous datasets and advanced modeling to disentangle true evolutionary sequences from apparent overlaps, ultimately connecting progenitor pathways to the observed diversity of explosions

</details>


### [35] [Constraints on Axion-Like Particles from VERITAS Observations of a Flaring Radio Galaxy in the Perseus Cluster](https://arxiv.org/abs/2510.19010)
*C. B. Adams,A. Archer,P. Bangale,J. T. Bartkoske,W. Benbow,Y. Chen,J. L. Christiansen,A. J. Chromey,A. Duerr,M. Errando,M. Escobar Godoy,J. Escudero Pedrosa,S. Feldman,Q. Feng,S. Filbert,L. Fortson,A. Furniss,W. Hanlon,O. Hervet,C. E. Hinrichs,J. Holder,Z. Hughes,T. B. Humensky,M. Iskakova,W. Jin,P. Kaaret,M. Kertzman,M. Kherlakian,D. Kieda,T. K. Kleiner,N. Korzoun,F. Krennrich,S. Kumar,S. Kundu,M. J. Lang,M. Lundy,P. Moriarty,R. Mukherjee,W. Ning,R. A. Ong,A. Pandey,M. Pohl,E. Pueschel,J. Quinn,P. L. Rabinowitz,K. Ragan,P. T. Reynolds,D. Ribeiro,E. Roache,C. Rulten,I. Sadeh,L. Saha,M. Santander,G. H. Sembroski,R. Shang,D. Tak,A. K. Talluri,J. V. Tucci,J. Valverde,V. V. Vassiliev,D. A. Williams,S. L. Wong,J. Woo,T. Yoshikoshi,M. Meyer*

Main category: astro-ph.HE

TL;DR: No evidence for axion-like particles (ALPs) was found in the 2017 NGC 1275 flare data. Constraints on ALP mass (∼2–4×10⁻⁷ eV) and coupling strength (g_{aγ} ≥ 3×10⁻¹¹ GeV⁻¹) exclude some parameter space up to 80% confidence, though no exclusion exceeds 95%. The study emphasizes the CLs method's reliability.


<details>
  <summary>Details</summary>
Motivation: To search for ALP-induced photon-ALP oscillation signals in TeV gamma-rays from NGC 1275, leveraging its location in the Perseus cluster's extended magnetic field. This tests ALP models in a unique astrophysical setting.

Method: Analysis of VERITAS gamma-ray data using gammapy for likelihood analysis with the conservative CLs method. Extensive fitting/models assessed potential ALP mixing effects through photon spectra modulations.

Result: No ALP evidence detected. Exclusions on mass/coupling parameters reach up to 80% CL in specific ranges, aligning with prior studies, but no >95% CL exclusions achieved.

Conclusion: CLs method proved reliable for setting ALP constraints. The study highlights NGC 1275's utility for future ALP searches needing improved sensitivity and larger datasets.

Abstract: Background: Axion-like particles (ALPs) are hypothetical particles that emerge in numerous theoretical extensions to the Standard Model. Their coupling to electromagnetic field implies that ALPs would mix with photons in the presence of external magnetic fields. As ALP phenomenology is governed by the mass and strength of its coupling, there is a subset of this parameter space in which this mixing would be expected to leave an imprint on the spectra of TeV gamma-ray sources.
  Data: In 2017, the VERITAS gamma-ray observatory recorded the second day of a dramatic flare of the radio galaxy NGC 1275, embedded at the center of the Perseus galaxy cluster. This serendipitous locale provides a spatially-extended magnetic field of strength O(10$μ$G) through which escaping photons traverse, making it an excellent target to study ALPs.
  Methods: We analyze the VERITAS data of NGC 1275's 2017 flare with the gammapy analysis package. Extensive fitting and modeling are performed to ultimately conduct a likelihood analysis used to search for any evidence of a preference for ALPs and to explore the confidence with which constraints can be set. We adopt the CLs method for this study for its conservative approach to setting limits in regimes where the search has limited sensitivity.
  Results: No evidence for the existence of ALPs is found, and no combination of mass and coupling strength can be excluded at or above 95% confidence level. We provide a map showing the strength of our exclusions in the mass and coupling parameter space. The strongest exclusions are found in the mass range $2 \times 10^{-7}$eV $\lesssim m_a \lesssim 4 \times 10^{-7}$eV and at the coupling strength of $g_{aγ} \gtrsim 3 \times 10^{-11}$ GeV$^{-1}$ up to 80% confidence level, which are consistent with previous studies.
  Conclusions: We find the CLs method to be a trustworthy approach, and advocate for its...

</details>


<div id='hep-ph'></div>

# hep-ph [[Back]](#toc)

### [36] [A Thermal Relic Encyclopedia: Dark Matter Candidates Coupled to Quarks](https://arxiv.org/abs/2512.03133)
*Dan Hooper,Gordan Krnjaic,Tanner Trickle,Isaac R. Wang*

Main category: hep-ph

TL;DR: The paper systematically explores dark matter and mediator particle combinations that can achieve thermal freeze-out, focusing on mediators coupling to quarks. It calculates relic densities and checks constraints from various experiments.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive framework for understanding thermal freeze-out as a mechanism for dark matter abundance and to catalog model parameters for future research.

Method: The authors analyze a wide range of dark matter and mediator models, compute relic densities, and assess constraints from accelerators, cosmology, direct/indirect detection across all mass ranges.

Result: A comprehensive catalog of matrix elements, cross sections, and decay rates for future dark matter studies was created. All viable models were constrained by existing experimental data.

Conclusion: Thermal freeze-out remains a robust mechanism for dark matter abundance when considering mediator-quark couplings. The catalog serves as a critical reference for phenomenological investigations.

Abstract: Thermal freeze-out is a compelling framework for naturally generating the dark matter abundance. We systematically study a broad range of dark matter and mediator particle combinations that can viably realize thermal freeze-out, focusing on models in which the mediator couples to Standard Model quarks. In each case, we calculate the relic density and consider existing constraints from accelerators, cosmology, direct detection, and indirect detection over the full range of dark matter and mediator masses. We present an encyclopedic catalog of matrix elements, cross sections, and decay rates which can be used as a reference for future studies of dark matter phenomenology.

</details>


### [37] [Muon collider probes of the gluonic quartic gauge couplings](https://arxiv.org/abs/2512.03144)
*Yu-Chen Guo,Peng-Cheng Lu,Tong Arthur Wu*

Main category: hep-ph

TL;DR: The paper proposes using the μ⁺μ⁻→ggγ process at future muon colliders to study dimension-8 gluonic quartic gauge couplings, demonstrating superior sensitivity compared to LHC, and combines experimental projections with theoretical positivity bounds.


<details>
  <summary>Details</summary>
Motivation: To probe dimension-8 gluonic quartic gauge couplings in the multi-TeV regime beyond LHC capabilities, leveraging muon collider advantages and theoretical constraints from positivity bounds.

Method: Event simulation and optimized kinematic cuts applied to μ⁺μ⁻→ggγ process; analysis of Wilson coefficient sensitivities and theoretical positivity bounds derived from QFT principles.

Result: Projected muon collider sensitivities surpass LHC bounds, reaching multi-TeV new-physics scales; combined experimental and theoretical constraints define stringent limits on Wilson coefficients.

Conclusion: The μ⁺μ⁻→ggγ process emerges as a leading probe for dimension-8 new physics at muon colliders, offering complementary insights through both experimental measurements and QFT-based positivity bounds.

Abstract: We investigate the dimension-8 gluonic quartic gauge couplings (gQGCs) at future high-energy muon colliders through the process $μ^{+}μ^{-}\!\to ggγ$. Using detailed event simulation and optimized kinematic selections, we derive projected sensitivities to the Wilson coefficients and their associated new-physics scales, showing that muon colliders can probe deep into the multi-TeV regime and significantly surpass current LHC bounds. We further present the positivity bounds on those Wilson coefficients, as theoretical constraints from the fundamental principles of quantum field theory. Our results establish $μ^{+}μ^{-}\!\to ggγ$ as one of the most sensitive probes of dimension-8 new physics at future muon colliders.

</details>


### [38] [Heavy Long-lived Dark Vector Via a Gluonic Portal](https://arxiv.org/abs/2512.03153)
*Xiaoyong Chu,Qiyuan Gao,Hongkai Liu,Teng Ma,Chengjie Yang*

Main category: hep-ph

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: We study a dark gauge boson $ Z' $ that exclusively couples to the QCD gluons through higher dimensional operators. These operators are generated from integrating out of heavy ultraviolet resonances carrying both QCD and dark gauge charges. With $ SU(3)_C $ gauge invariance, charge and parity symmetries preserved, we find that the leading effective operators are restricted to have the form of $ Z'GGG $ and $ Z'Z'GG $ at dimension-eight, which can naturally render the $Z^\prime$ particle long-lived, and serve as a viable dark matter candidate. We investigate the phenomenology of these operators with both collider experiments and cosmological observation, without and with the assumption that this dark gauge boson plays the role of the dominant dark matter component. For an unstable $Z'$, we show that depending on its lifetime, it can be probed by various observables up to ultraviolet physics scale around $10^9$ GeV. For $Z'$ being dark matter, we find that $m_{Z^\prime} \gtrsim 1 $ TeV is consistent with the thermal freeze-out scenario. In contrast, in the freeze-in scenario, the extremely small couplings leave the relevant parameter space largely unconstrained by current experiments.

</details>


### [39] [Effective Field Theory Perspective On King Non-linearity](https://arxiv.org/abs/2512.03157)
*Benoît Assi,Sam Carey,Sebastian Jäger,Gabriel Lee,Gil Paz,Gilad Perez,Jure Zupan*

Main category: hep-ph

TL;DR: The paper develops an effective field theory framework to systematically analyze King non-linearity in isotope-shift experiments, separating Standard Model contributions and providing a clearer classification of nuclear effects for precision measurements.


<details>
  <summary>Details</summary>
Motivation: To address the lack of control over Standard Model contributions in interpreting isotope shift data and to improve understanding of heavy nuclei structure through a more rigorous theoretical framework.

Method: The authors use a systematic effective field theory approach, matching the SM onto scalar non-relativistic QED in the infinite nuclear mass limit, then onto quantum-mechanical potentials. This separates short- and long-distance physics via Wilson coefficients, re-examines the treatment of ⟨r²⟩² terms, and derives a 1/r⁴ potential from nuclear polarizability.

Result: Shows that ⟨r²⟩² contributions must be treated at second-order perturbation, identifies long-range 1/r⁴ potentials from polarizability, and provides a transparent classification of SM sources of King non-linearity for hydrogen-like systems.

Conclusion: The framework enhances precision in interpreting isotope shift experiments, enabling higher precision studies of heavy nuclei shapes and potentially aiding searches for fifth-force mediators.

Abstract: Precision spectroscopic measurements of isotope shifts have recently reached a high level of accuracy. Tests of King non-linearity (NL) along isotope chains have been proposed as a tool to search for fifth-force mediators. At the same time, these tests can potentially teach us about the structure of heavy nuclei at unprecedented precision, where King NL has already been observed in several systems. A robust interpretation of the existing data, however, is hampered by incomplete control over the Standard Model (SM) contributions. We develop a systematic effective field theory framework, matching the SM onto scalar non-relativistic QED in the infinite nuclear mass limit and then onto quantum-mechanical potentials. This approach organizes all nuclear effects into a small set of Wilson coefficients and cleanly separates short- and long-distance physics. We show that the commonly used treatment of the $\langle r^2\rangle^2$ term needs to be reconsidered, as it arises only at second-order in perturbation theory, and we derive the long-range $1/r^4$ potential from nuclear polarizability. Applying the framework to hydrogen-like systems, we provide a transparent classification of SM sources of King NL relevant for current and future isotope-shift experiments. The formalism can be applied to learn about the shape of the heavy scalar nuclei at a higher level of precision and detail than what was previously attainable.

</details>


### [40] [New physics in toponium's shadow?](https://arxiv.org/abs/2512.03220)
*Thomas Flacke,Benjamin Fuks,Dongchan Kim,Jinheung Kim,Seung J. Lee,Léandre Munoz-Aillaud*

Main category: hep-ph

TL;DR: The paper examines how a hypothetical neutral pseudoscalar particle, interacting with gluons and top quarks, could influence the enhanced top-antitop production observed near threshold by ATLAS and CMS. It combines Standard Model effects with new physics to constrain the pseudoscalar's parameter space, suggesting compatibility with both scenarios.


<details>
  <summary>Details</summary>
Motivation: To explain the observed enhancements in top-antitop production near threshold, the authors explore the role of a neutral pseudoscalar particle, considering both Standard Model non-perturbative QCD effects and potential new physics contributions.

Method: An effective theory approach incorporating perturbative SM contributions, new physics (pseudoscalar coupling), their interference, and non-perturbative threshold dynamics. Analyzes parameter space viability for pseudoscalars near twice the top mass.

Result: The combined effects significantly constrain pseudoscalar parameter space, especially for narrow resonances. While SM threshold effects alone can explain some enhancements, current data still allow for contributions from the pseudoscalar.

Conclusion: The results suggest that observed top-pair enhancements could originate from either SM threshold effects or new physics via pseudoscalar interactions, with ongoing compatibility between data and both explanations.

Abstract: ATLAS and CMS have recently reported enhancements in the top-antitop production rate near threshold, a region where non-perturbative QCD dynamics associated with toponium formation become relevant. We investigate how this behaviour is modified in the presence of a neutral pseudoscalar that couples to gluons and top quarks, using an effective description that consistently incorporates perturbative Standard Model and new physics contributions, their interference and non-perturbative threshold effects. We show that the combined effect of those ingredients markedly shapes the viable region of the pseudoscalar parameter space, particularly for narrow resonances with masses close to twice the top mass. While Standard Model threshold effects could explain a sizeable part of the measured enhancements, the current data remain compatible with additional contributions from pseudoscalar interactions.

</details>


### [41] [NNLO+NNLL Predictions for Heavy-Jet Mass and C-parameter in Higgs Decays to Quarks and Gluons](https://arxiv.org/abs/2512.03263)
*Elliot Fox,Aude Gehrmann-De Ridder,Thomas Gehrmann,Nigel Glover,Matteo Marcoli,Christian T. Preuss*

Main category: hep-ph

TL;DR: The paper presents a resummation analysis of large logarithmic corrections at NNLL accuracy for Higgs boson decay distributions into heavy quarks and gluons, showing reduced uncertainties and significant effects on the C-parameter peak in gluonic decays.


<details>
  <summary>Details</summary>
Motivation: To clarify the contributions of different hadronic Higgs-decay channels and reduce uncertainties in event-shape observables like heavy-jet mass and C-parameter.

Method: Resummation of large logarithmic corrections at NNLL accuracy, combined with NNLO calculations, applied to H→bb̄, cc̄, and gg decay processes.

Result: Matched NNLO+NNLL results reduced uncertainties, particularly for heavy-jet mass, and revealed substantial shifts in the C-parameter peak for gluonic decays.

Conclusion: The analysis improves precision in Higgs decay studies by systematically addressing logarithmic corrections, emphasizing the role of gluonic channels in modifying event-shape distributions.

Abstract: We consider the resummation of large logarithmic corrections arising in the two-particle limit at next-to-next-to-leading logarithmic (NNLL) accuracy for the heavy-jet mass and $C$-parameter distributions in the decay of a Higgs boson to quarks and gluons: $H\to b\bar{b}$, $H\to c\bar{c}$, and $H\to gg$. We demonstrate how the matched NNLO+NNLL results clarify the relative contributions of key hadronic Higgs-decay channels ($H\to b\bar{b}$, $H\to c\bar{c}$, $H\to gg$) yielding reduced uncertainties for both event-shape observables -- especially for heavy-jet mass -- while revealing substantial effects that shift the $C$-parameter peak in gluonic decays.

</details>


### [42] [Lepton number violation from Higgs/$Z$ decays into a pair of right-handed neutrinos](https://arxiv.org/abs/2512.03302)
*Arindam Das,Takaaki Nomura,Kei Yagyu*

Main category: hep-ph

TL;DR: The paper investigates signatures of Lepton Number Violation (LNV) from Higgs and Z boson decays into Right-Handed Neutrinos (RHNs), focusing on same-sign dilepton plus jets signatures. Using a U(1)_X gauge symmetry model with RHNs, it evaluates LNV signals and background constraints at various colliders, deriving bounds on mixing parameters.


<details>
  <summary>Details</summary>
Motivation: To explore LNV signatures in existing and future collider experiments using Higgs/Z decays into RHNs, which can test beyond-Standard Model scenarios like U(1)_X gauge extensions.

Method: Calculate Higgs-Z' and Z-Z' mixing-induced decay processes, simulate $	au^{\pm}\tau^{\pm}4j$ signal and background events, apply analysis cuts, and derive mixing parameter bounds for different collider setups.

Result: Set constraints on scalar (h-φ) and gauge (Z-Z') mixing vs. RHN mass at HL-LHC (14 TeV), FCC-ee (91.2 GeV), and others. Highlight sensitivity differences between electron-muon and muon colliders.

Conclusion: Collider searches for LNV Higgs/Z decays provide complementary constraints on RHN mixing parameters in U(1)_X models, with optimal sensitivity at specific collision energies and luminosities.

Abstract: We explore signatures of Lepton Number Violation (LNV) from decays of the discovered Higgs ($h$) and Z bosons into a pair of Right-Handed Neutrinos (RHNs). Due to the Majorana nature of RHNs, the final state can be the same-sign dilepton plus jets leading to 2 units of LNV. As a simple but plausible scenario, we investigate such a signal in models with a new $U(1)_X$ gauge symmetry which naturally introduces three RHNs for gauge anomaly cancellation, and is spontaneously broken down by a vacuum expectation value of an isospin singlet scalar field ($φ$). In this scenario, $h$ and the Z boson can decay into a pair of RHNs via the $h$-$φ$ mixing and the $Z$-$Z'$ mixing with $Z'$ being a new massive gauge boson, respectively. Estimating the LNV signal and corresponding backgrounds for the $\ell^{\pm} \ell^{\pm} 4j$ final states, we find bounds on the $h$-$φ$ mixing and the $Z$-$Z^\prime$ mixing as a function of a mass of RHNs at Higgs factories, e.g., at High-Luminosity LHC with $\sqrt{s}=$ 14 TeV, future $e^-e^+$ colliders at $\sqrt{s}=$ 250 GeV, muon colliders at $\sqrt{s}=$ 125 GeV as well as at Z factories, e.g., CEPC and FCC-ee at $\sqrt{s}=$ 91.2 GeV. We also discuss limits on the scalar mixing at $e^-μ^+$ and $μ^+ μ^+$ collisions ($μ$TRISTAN) with $\sqrt{s}=$346 GeV/775 GeV and 2 TeV, respectively.

</details>


### [43] [Probing the Electroweak Phase Transition in the Dark Sector](https://arxiv.org/abs/2512.03567)
*Maimoona Razzaq,Nico Benincasa,Luigi Delle Rose,Luca Panizzi*

Main category: hep-ph

TL;DR: This paper explores an extended Standard Model with a dark SU(2) gauge group, introducing a dark scalar doublet interacting via a portal coupling with the Standard Model Higgs. The model stabilizes dark gauge bosons as dark matter candidates and identifies parameter regions compatible with observed relic density and gravitational-wave signals detectable by future missions.


<details>
  <summary>Details</summary>
Motivation: To address the dark matter problem and explore the potential for gravitational-wave detection by extending the Standard Model with a dark sector that ensures viable dark matter candidates and strong phase transitions.

Method: The authors introduce a dark SU(2) gauge group with a scalar doublet, enforce a custodial symmetry to stabilize dark gauge bosons, and perform parameter space scans considering collider and astrophysical constraints to find regions matching observed relic density and phase transitions.

Result: Regions of parameter space were found that satisfy relic density requirements and produce gravitational-wave signals within detection range of upcoming space-based detectors.

Conclusion: The model provides a viable framework for dark matter and electroweak phase transition studies, with detectable gravitational-wave signatures poised to be observed in future experiments.

Abstract: We study an extension of the Standard Model with a dark SU(2) gauge group, where a dark scalar doublet interacts with the Standard Model Higgs through a portal coupling, inducing mixing after symmetry breaking. A custodial symmetry ensures the stability of the dark gauge bosons, making them viable dark matter candidates. Scanning the parameter space of the model under collider and astrophysical constraints, we find regions that yield the observed relic density and strong first-order phase transitions. The resulting gravitational-wave signals fall within the reach of upcoming space-based detectors.

</details>


### [44] [Real-time evolution of critical modes in the QCD phase diagram](https://arxiv.org/abs/2512.03614)
*Yang-yang Tan,Shi Yin,Yong-rui Chen,Chuang Huang,Wei-jie Fu*

Main category: hep-ph

TL;DR: The paper presents a QCD-assisted relaxation dynamic model to study critical slowing down near the CEP in the QCD phase diagram, revealing a narrow dynamic critical region with significant relaxation time enhancement near the CEP, while freeze-out curves show mild effects.


<details>
  <summary>Details</summary>
Motivation: To quantitatively analyze the critical slowing down effect near the CEP without phenomenological parameters, extending understanding of dynamic behavior in the QCD phase diagram.

Method: Developed a QCD-assisted relaxation dynamic model and performed Langevin simulations to extract relaxation times across the QCD phase diagram.

Result: Identified a narrow region near the CEP with significantly prolonged relaxation times, indicating a small dynamic critical region; critical slowing down effects are weak on chemical freeze-out curves.

Conclusion: The model successfully quantifies critical slowing down dynamics, highlighting the localized nature of criticality near the CEP and its limited influence on freeze-out processes.

Abstract: A QCD-assisted relaxation dynamic model for the critical mode of the critical end point (CEP) in the QCD phase diagram is developed, which allows us to investigate the critical slowing down effect quantitatively in the QCD phase diagram, especially in the proximity of the CEP, without any phenomenological parameters. The relaxation time from nonequilibrium to equilibrium in the QCD phase diagram is extracted from the Langevin simulations of the QCD-assisted relaxation dynamic model. It is found that in a narrow region along the phase boundary radiated from the CEP, the relaxation time is enhanced significantly. Outside this narrow region, the relaxation time drops drastically, which implies that the dynamic critical region is small in the QCD phase diagram. We also find that the effects of critical slowing down are mild on the chemical freeze-out curves.

</details>


### [45] [Gravitational Decays of Secluded Scalars and Graviton Dark Radiation](https://arxiv.org/abs/2512.03662)
*Kazunori Nakayama,Fuminobu Takahashi,Juntaro Wada*

Main category: hep-ph

TL;DR: The paper explores graviton dark radiation from the decay of a secluded scalar field, specifically the dark glueball, which couples to the Standard Model via gravity. It shows that when the Higgs has a strong gravitational coupling, graviton production is suppressed, favoring reheating of the Standard Model sector and reducing energy transfer to hidden sectors. They also predict the gravitational-wave spectrum from this scenario.


<details>
  <summary>Details</summary>
Motivation: To investigate the impact of secluded scalar field decays, particularly dark glueballs, on the early universe's thermal history through their graviton decay products and resulting dark radiation. This addresses how such particles' interactions with gravity affect energy distribution between sectors and observable phenomena like gravitational waves.

Method: Analyzing decay rates of the secluded scalar (dark glueball) into both Standard Model particles (Higgs, gluons) and gravitons, considering non-minimal gravitational couplings. The study evaluates how conformal invariance breaking influences graviton production and thermal history, using metric formalism to model interactions.

Result: Decays into SM particles (Higgs/gluons) dominate over gravitons when Higgs has large non-minimal gravity coupling, suppressing graviton dark radiation. This prioritizes SM reheating and limits energy transfer to hidden sectors. The gravitational-wave spectrum from dark glueball domination is derived.

Conclusion: Secluded scalars like dark glueballs with significant gravitational couplings can naturally reduce unwanted dark radiation, aligning cosmological predictions with observations. Their decay dynamics provide testable gravitational-wave signatures for future experiments.

Abstract: We discuss graviton dark radiation produced by the decay of a secluded scalar field that couples to the Standard Model (SM) only through gravity. Such scalar fields are long-lived, and their decay channels generically include gravitons. If such particles existed and dominated the early universe, a sizable branching ratio into gravitons would yield non-negligible dark radiation that significantly alters the subsequent thermal history of the universe. In this work, we focus on the dark glueball as a representative secluded hidden scalar and compare the decay rates into SM particles via a non-minimal coupling to gravity with those into gravitons, paying attention to how the breaking of conformal invariance affects the amount of graviton dark radiation. We find that decays into the SM are dominated by two-body decay channels into Higgs bosons and gluons. In particular, when the Higgs field has a large non-minimal coupling to gravity, the production of graviton dark radiation is naturally suppressed in the metric formalism, and the SM sector is preferentially reheated and energy transfer to other hidden sectors is suppressed. Finally, we present the expected gravitational-wave spectrum resulting from dark glueball domination.

</details>


### [46] [Probing Azimuthal Alignment in Heavy-Ion Collisions: Clusterization Effects](https://arxiv.org/abs/2512.03725)
*Aleksei Nikolskii,Igor Lokhtin,Alexander Snigirev*

Main category: hep-ph

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: The influence of kinematic constraints and event selection on the emergence of the alignment phenomenon observed in cosmic-ray experiments is studied within the HYDJET++ model. It is demonstrated that the high degree of alignment, previously identified for realistic values of the transverse momentum disbalance of the most energetic particles, is also observed at the level of the most energetic clusters. In high-multiplicity events, the clustering procedure plays a crucial role in resolving individual particle groups on the detection plane, allowing a more accurate characterization of alignment patterns. These results highlight the combined effects of cluster formation and momentum conservation in shaping the observed azimuthal correlations.

</details>


### [47] [Hidden-charm and -bottom tetraquark states with $J^{PC}=1^{-+}$ via QCD sum rules](https://arxiv.org/abs/2512.03800)
*Bing-Dong Wan,Yan Zhang,Jun-Hao Zhang,Ming-Yang Yuan*

Main category: hep-ph

TL;DR: The study uses QCD sum rules to predict masses of hidden-charm and hidden-bottom tetraquark states, identifying four possible states in each sector with specific mass values, and suggests experimental verification at BESIII, Belle II, and LHCb.


<details>
  <summary>Details</summary>
Motivation: To explore exotic tetraquark states with $1^{-+}$ quantum numbers and guide experimental searches by calculating their masses and decay modes using QCD sum rules.

Method: QCD sum rules framework incorporating condensates up to dimension eight in operator product expansion to compute mass spectra.

Result: Predicted masses for four hidden-charm states (4.72-4.88 GeV) and four hidden-bottom states (10.99-11.16 GeV), along with proposed decay modes for experimental detection.

Conclusion: Findings offer crucial theoretical insights to aid future experiments in discovering these exotic tetraquark states in charm and bottom sectors.

Abstract: We investigate the $1^{-+}$ hidden-charm and hidden-bottom tetraquark states within the framework of QCD sum rules. The mass spectra are computed by including condensates up to dimension eight in the operator product expansion. Our results indicate the possible existence of four $1^{-+}$ hidden-charm tetraquark states, with predicted masses of $(4.83 \pm 0.15)$ GeV, $(4.88 \pm 0.18)$ GeV, $(4.72 \pm 0.16)$ GeV, and $(4.79 \pm 0.12)$ GeV, while their hidden-bottom counterparts are estimated to have masses of $(11.08 \pm 0.16)$ GeV, $(11.16 \pm 0.14)$ GeV, $(10.99 \pm 0.16)$ GeV, and $(11.03 \pm 0.15)$ GeV, respectively. We also analyze the possible decay modes of these tetraquark states, which may be accessible in future experiments at BESIII, Belle~II, and LHCb. These findings provide valuable guidance for the experimental search for exotic $1^{-+}$ tetraquark states in both the charm and bottom sectors.

</details>


### [48] [A strategy for optimal material identification in solar dark photon absorption](https://arxiv.org/abs/2512.03801)
*Theresa M. Backes,Riccardo Catena,Michael Krämer*

Main category: hep-ph

TL;DR: The paper proposes a method to optimize detector materials for detecting solar dark photons in the 1-100 eV mass range by using Kramers-Kronig relations to establish absorption rate limits and analyzing material properties relative to plasma frequencies.


<details>
  <summary>Details</summary>
Motivation: To identify optimal detector materials for solar dark photon absorption, providing a model-independent framework for next-gen experiments probing hidden-sector physics beyond the Standard Model.

Method: Develop a strategy using Kramers-Kronig relations applied to longitudinal/transverse dark photon modes, derive material-independent upper limits on absorption rates, and evaluate existing materials and metamaterials with tunable plasma frequencies.

Result: The framework identifies optimal material properties based on dark photon mass vs detector plasma frequency, shows conditions for saturating upper limits, and highlights metamaterials' potential for tailored detection.

Conclusion: The strategy offers a general guideline for designing detectors targeting solar dark photons, enhancing experimental efforts in hidden-sector physics beyond the Standard Model.

Abstract: Dark photons with masses in the 1-100 eV range can be produced in the Sun and subsequently absorbed in terrestrial detectors, offering a promising avenue for probing hidden-sector physics beyond the Standard Model. In this work, we develop a theoretically grounded strategy to identify optimal detector materials for solar dark photon absorption. Our strategy builds on a material-independent upper limit on the absorption rate, which we derive from Kramers-Kronig relations applied separately to the longitudinal and transverse dark photon modes. We show how the optimal material properties depend on the dark photon mass relative to the detector's plasma frequency, identifying the conditions under which a detector can saturate the theoretical upper limit. We then assess the performance of commonly used detector materials in light of these criteria and comment on the prospects of metamaterials featuring tunable plasma frequencies. Our results provide a general and model-independent framework to effectively guide the design of next-generation experiments targeting solar dark photons.

</details>


### [49] [Discrete flavour and CP symmetries in light of JUNO and neutrino global fit](https://arxiv.org/abs/2512.03809)
*Gui-Jun Ding,Cai-Chang Li,Jun-Nan Lu,S. T. Petcov*

Main category: hep-ph

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: Working within the reference three-neutrino mixing framework, we confront the lepton mixing predictions derived using non-Abelian discrete flavour and CP symmetries with the firs JUNO data on the solar neutrino mixing parameters $\sin^2θ_{12}$ and with the results of the latest global neutrino data analysis. We focus on symmetry breaking patterns for which the lepton PMNS mixing matrix depends only on one or two free real parameters. Performing a comprehensive statistical analyses in each of the considered cases, we report the best fit values, the $3σ$ C.L. allowed ranges and the $χ^2$-distributions of the lepton mixing observables - the three mixing angles and the three CP-violation phases. We find that the JUNO measurements can disfavour or rule out a number of the mixing patterns associated with specific types of breakings of the discrete flavour and CP symmetries. The synergy of JUNO, DUNE and T2HK data can provide an exhaustive test of the considered approach to lepton mixing based on non-Abelian discrete lepton flavour symmetries combined with the CP symmetry.

</details>


### [50] [Production of charged Higgs bosons associated with CP-even Higgs bosons at future multi--TeV muon colliders](https://arxiv.org/abs/2512.03810)
*Quang Hoang-Minh Pham,Khoa Ngo-Thanh Ho,Khiem Hong Phan*

Main category: hep-ph

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: We report the first results for charged Higgs boson production associated with CP-even Higgses at future multi--TeV muon colliders within the scenario of the Type-Y Two-Higgs-Doublet Model (THDM). The valid parameter regions in the Type-Y THDM are first updated, and based on the allowed parameter space, all two-body decay channels of the charged Higgs boson are then computed with the help of {\tt H-COUP}. Both production processes $μ^- μ^+ \to H^\pm H^\mp h, H^\pm H^\mp H \to t\bar{t} b\bar{b} h, t\bar{t} b\bar{b}H$ are scanned over the constrained parameter space of the considered model. The production cross sections can reach $\sim 0.5$ fb in several regions of the parameter space. The results provide an opportunity to probe charged Higgs bosons produced in association with the heavy CP-even Higgses at the high integrated luminosities proposed for future multi--TeV muon colliders. Furthermore, we evaluate the signal significances for $μ^- μ^+ \to H^\pm H^\mp h \to t\bar{t} b\bar{b} h \to t\bar{t} b\bar{b} b\bar{b}$ with respect to the Standard Model backgrounds. We also estimate the significances by including the top quark decay into leptons and bottom quarks and considering $b$-tagging. Thanks to the high integrated luminosities anticipated at future multi--TeV muon colliders,we find that the signal significances can exceed $\sim 3σ$ at several benchmark points selected in the viable parameter space of the Type-Y THDM.

</details>


### [51] [Kinetic Mixing and Axial Charges in the Parity-Doublet Model](https://arxiv.org/abs/2512.03894)
*Christian Kummer,Stefan Leupold,Lorenz von Smekal*

Main category: hep-ph

TL;DR: The standard parity doublet model underpredicts the nucleon axial charge $g_A$. We introduce kinetic-mixing terms to address this, extending the model with five parameters, three determined by empirical data.


<details>
  <summary>Details</summary>
Motivation: The standard model fails to reproduce the observed $g_A \approx 1.28$ due to mass mixing causing $g_A < 1$. The proposed kinetic-mixing terms, inspired by $η$-$η'$ mixing, aims to resolve this discrepancy. Additionally, the paper addresses parameter determination in the extended model.

Method: Extending the parity doublet model with meson-baryon derivative couplings (kinetic mixing). Parameters are constrained using nucleon $g_A$, nucleon mass, and $N^*(1535)$ mass. Remaining parameters are addressed via chiral condensate conditions, nucleon mass in chiral limit, and meson-baryon decay couplings.

Result: The extended model accommodates the empirical $g_A$ value. Three parameters (related to masses and $g_A$) are empirically determined, while others require further analysis through additional physical quantities.

Conclusion: Kinetic mixing effectively resolves the axial charge discrepancy in the parity doublet model. Further experimental input is needed to fully constrain the model's parameters, emphasizing the role of meson-baryon interactions and chiral symmetry considerations.

Abstract: The standard parity doublet model with its mass-mixing mechanism fails to describe the axial charge $g_A$ of the nucleon. While $g_A = 1$ in the original Gell-Mann--Levy model, which reproduces the Adler-Bell-Jackiw anomaly of QCD, in the presence of a chirally invariant baryon mass the mass mixing leads to $g_A < 1 $ whereas phenomenologically it is about 1.28. We propose to remedy this problem by introducing kinetic-mixing terms corresponding to meson-baryon derivative couplings, similar in spirit to the two-mixing-angle scenario of the $η$-$η'$ mixing. This extended parity doublet model contains five parameters in the effective baryonic Lagrangian. Three of them can be determined by using the empirical results for the axial charge of the nucleon together with the masses of the nucleon and its parity partner, the $N^*(1535)$ resonance. We discuss various options how to determine the remaining parameters, touching upon the mass of both parity partners if the chiral condensate is put to zero; the mass of the nucleon in the chiral limit; and the values of meson-baryon coupling constants related to the decays of the resonance to pion-nucleon and sigma-nucleon.

</details>


### [52] [Jet Production at NNLO: Exploring a New Scheme](https://arxiv.org/abs/2512.03954)
*Luca Buonocore,Massimiliano Grazzini,Flavio Guadagni,Jürg Haag,Stefan Kallweit,Luca Rottoli*

Main category: hep-ph

TL;DR: The paper presents a new non-local subtraction scheme to calculate the fully differential cross section for dijet production in $e^+e^-$ collisions and $H\to b\bar{b}$ decays at NNLO QCD, comparing slicing variables for precision cross section calculations.


<details>
  <summary>Details</summary>
Motivation: To improve precision in perturbative QCD calculations by developing and applying a novel subtraction method for complex processes like dijet production and Higgs decays.

Method: Introduces a new non-local subtraction scheme at NNLO to handle real-virtual emission contributions. The method is applied to compute fully differential cross sections, with comparisons of slicing variables for evaluating corrections.

Result: Successfully applied the subtraction scheme for the first time in these processes, providing precise NNLO predictions and insights into optimal slicing variable performance.

Conclusion: The new approach enhances accuracy in QCD calculations, enabling reliable predictions for high-energy physics experiments and validating slicing techniques for future studies.

Abstract: We consider dijet production in $e^+e^-$ collisions and in $H\to b{\bar b}$ decays at next-to-next-to-leading order (NNLO) in perturbative QCD. A new non-local subtraction scheme is applied, for the first time, to obtain the fully differential cross section for these benchmark processes. We discuss and explicitly evaluate the perturbative ingredients needed in the computation, and we compare the performance of different slicing variables to obtain the NNLO corrections.

</details>


### [53] [Superexotic $K^{*+}D^{*+}K^{*+}$ bound state](https://arxiv.org/abs/2512.04001)
*Wen-Hao Jia,Pei-Shen Su,Wei-Hong Liang,Raquel Molina,Eulogio Oset*

Main category: hep-ph

TL;DR: The paper investigates a highly exotic system composed of three particles ($K^{*+}D^{*+}K^{*+}$) with unique quantum numbers, finding a bound three-body state with a binding energy of ~100 MeV and narrow width (~10 MeV), suggesting experimental verification through specific measurements.


<details>
  <summary>Details</summary>
Motivation: To explore exotic hadronic states beyond conventional meson structures, specifically a triply Charmed system with high charge, isospin, spin, and unconventional quark composition, aiming to discover new particle states and understand strong interactions at higher energies.

Method: The authors analyze the interaction of a three-body system formed by a $K^{*+}D^{*+}$ cluster (in I=1, J=2) and an additional $K^{*+}$ particle with aligned spins. They calculate the binding energy by comparing the system's mass to the sum of individual particle masses and estimate the width from interaction strengths. The $K^*K^*$ interaction is considered repulsive but weaker than the cluster interaction.

Result: A bound state with a binding energy of ~100 MeV relative to the $K^{*+}$ and $K^{*+}D^{*+}$ cluster is identified. The state has a narrow width (~10 MeV), indicating potential observability in experiments.

Conclusion: The study proposes that this exotic state can be detected via invariant mass measurements of $K D K^*$ particles in existing experimental setups, offering a pathway to confirm the existence of this novel hadronic configuration and deepen understanding of multi-quark systems.

Abstract: We study a system made from $K^{*+}D^{*+}K^{*+}$ with charge $3$, isospin $I=3/2$, spin $J=3$, and a quark content of $c\bar d \bar s u \bar s u$, which make it highly exotic relative to the standard $q\bar q$ structure of mesons. The interaction of the three body system is obtained starting from a cluster of $K^{*+}D^{*+}$ in $I=1$ and $J=2$, that in different works has been found bound, and adding to it an extra $K^{*+}$ with spin aligned with those of the vectors of the cluster. We find that the $K^* K^*$ interaction in $I=1$ and $J=2$ is repulsive, but its strength is small compared to that of $K^{*+}D^{*+}$ in $I=1$ and $J=2$, such that we find a three body state bound by about $100 \, \rm MeV$ with respect to the mass of a $K^{*+}$ and the $K^{*+} D^{*+}$ cluster. The width of the state, of about $10\, \rm MeV$, is much smaller than the binding, which facilitates its observation. We suggest to find that state by measuring the invariant mass of $K D K^*$, something feasible in present experimental facilities.

</details>


### [54] [New insights into two-loop running in effective field theories](https://arxiv.org/abs/2512.04064)
*Mikael Chala,Javier López Miras*

Main category: hep-ph

TL;DR: The paper proposes a method to compute two-loop anomalous dimensions in 4D effective field theory by relating them to infrared effects in a 5D compactified theory, avoiding traditional complications. This approach is used to cross-validate existing results and calculate new anomalous dimensions in the SMEFT and critical exponents in the Abelian Higgs model.


<details>
  <summary>Details</summary>
Motivation: To develop a simpler method for calculating two-loop anomalous dimensions in 4D theories without resorting to cumbersome techniques like gauge-breaking terms, IR rearrangement, or geometric methods. The motivation also includes extending current understanding by computing new SMEFT coefficients and critical exponents in specific models.

Method: By treating the 4D effective-field theory as the IR limit of a 5D compactified theory, the authors map UV divergences in 4D to IR effects in 5D. This avoids the need for gauge-breaking counter-terms or geometric techniques. The method involves matching calculations between 5D and 4D frameworks to extract required anomalous dimensions.

Result: The approach successfully cross-checks recent SMEFT results and calculates novel two-loop anomalous dimensions up to dimension-eight operators. It also determines critical exponents in the charged fixed point of the Abelian Higgs model at large flavor numbers, demonstrating the method's utility beyond mere replication of prior work.

Conclusion: The proposed 5D-to-4D approach provides a robust, efficient alternative for computing higher-loop anomalous dimensions. It confirms existing results while opening avenues for new calculations in SMEFT and strongly coupled systems, suggesting broader applications in theoretical physics frameworks involving extra dimensions or effective field theories.

Abstract: We show that, by viewing a 4D effective-field theory as the infrared (IR) limit of the compactified version in 5D, we can compute two-loop anomalous dimensions without gauge-breaking counter-terms, IR re-arrangement or geometric methods. The ultraviolet (UV) divergences in 4D are read from the IR ones in the matching from 5D to 4D. We use this approach to cross-check recent results in the literature, as well as to compute novel two-loop anomalous dimensions in the SMEFT to dimension eight and certain critical exponents in the charged fixed point of the Abelian Higgs model at large number of flavors.

</details>


### [55] [Flavour and precision probes of a class of scotogenic models](https://arxiv.org/abs/2512.04079)
*A. Darricau,H. Lee,J. Orloff,A. M. Teixeira*

Main category: hep-ph

TL;DR: The paper examines the effects of a specific type of scotogenic model on flavor and electroweak precision observables. It focuses on the 'T1-2-A' variant, calculating next-to-leading order corrections for leptonic Higgs and Z-boson decays. The study updates previous findings on operator dominance and parameter space constraints considering changes in Δa_μ, and evaluates how H→μμ decays and other observables can probe the model at future colliders.


<details>
  <summary>Details</summary>
Motivation: To investigate how scotogenic models influence flavor physics and electroweak precision tests, updating earlier conclusions with newer data on Δa_μ and identifying viable avenues for experimental verification at future colliders.

Method: The authors performed a next-to-leading order computation of leptonic Higgs and Z-boson decays in the T1-2-A scotogenic model variant. They analyzed operator dominance and parameter constraints, incorporating updated Δa_μ values. They also assessed the role of H→μμ decays and other flavor-conserving Higgs processes in probing the model.

Result: The study provides updated constraints on the model's parameter space based on refined Δa_μ considerations and identifies H→μμ decays along with other flavor-conserving Higgs observables as critical probes for future collider experiments.

Conclusion: The T1-2-A scotogenic model's viability is subject to stringent constraints from updated electroweak precision data and Δa_μ. Future colliders can effectively test the model through precise measurements of Higgs and Z-boson decays, particularly focusing on muon-related flavor-conserving channels.

Abstract: We address the phenomenological impact of a well-motivated class of scotogenic models regarding flavour and electroweak precision observables. For the case of a ``T1-2-A'' variant, we carry out a full computation of the next-to-leading order corrections to leptonic Higgs and $Z$-boson decays. We revisit previously drawn conclusions on operator dominance and constraints on the parameter space in view of the evolution regarding $Δa_μ$. Finally, we consider the role of $H \to μμ$ decays (and other flavour conserving Higgs decays), as well as precision observables in probing this class of models at future colliders.

</details>


<div id='astro-ph.IM'></div>

# astro-ph.IM [[Back]](#toc)

### [56] [Spectral Characterization of a 90 GHz CLASS Pixel](https://arxiv.org/abs/2512.03260)
*Gregory Jaehnig,John Appel,Sarah Marie Bruno,Jake Connors,Shannon M. Duff,Naina Gupta,Johannes Hubmayr,Matthew A. Koc,Tammy Lucas,Tobias Marriage,Lola Morales Perez,Caleigh Ryan,Jeff Van Lanen*

Main category: astro-ph.IM

TL;DR: The CLASS experiment aims to measure cosmic microwave background polarization to study cosmic reionization and inflationary B-modes. The study characterizes frequency responses of 90 GHz detectors, confirming -3 dB bandwidth at 80-108 GHz using novel methods.


<details>
  <summary>Details</summary>
Motivation: To probe cosmic reionization and search for inflationary B-mode signals via precise CMB polarization measurements, requiring accurate frequency response characterization of detector components.

Method: Simulations and measurements of 90 GHz band single pixel witness devices with/without bandpass filters. Fourier transform spectroscopy and design splits were used to isolate filter contributions from other microwave components.

Result: Measured -3 dB band edges at 80 GHz (lower) and 108 GHz (upper). Demonstrated a robust method to characterize individual component spectral responses.

Conclusion: The validated method is critical for optimizing future detector arrays' performance, ensuring accurate CMB polarization measurements needed for cosmological studies.

Abstract: The Cosmology Large Angular Scale Surveyor (CLASS) is an experiment designed to measure the polarization of the cosmic microwave background on large angular scales to probe cosmic reionization and search for the inflationary $B$-mode signal. CLASS is a multi-frequency ensemble of telescopes with bands centered at 40, 90, 150, and 220 GHz. Each telescope has arrays of feedhorn-coupled transition edge sensor bolometers at the focal plane. The frequency response is primarily defined by the on-chip bandpass filter with additional contributions coming from the feedhorn, orthomode transducer, and 180-degree hybrid. In this study, we compare simulations and measurements of the frequency response of single pixel witness devices in the 90 GHz band with and without the bandpass filter. For the first time, we can separate the effects of the bandpass filter from the other microwave components using Fourier transform spectroscopy and design splits of the pixel. The results show that the -3 dB band edges are at 80 GHz and 108 GHz. The measurements demonstrate a robust method for characterizing the spectral response of individual components, which is crucial for optimizing the performance of future detector arrays.

</details>


### [57] [Push-broom Mapping of Galaxies and Supernova Remnants with the SPRITE CubeSat](https://arxiv.org/abs/2512.03329)
*Elena Carlson,Brian Fleming,Yi Hang Valerie Wong,Briana Indahl,Dmitry Vorobiev,Maitland Bowen,Donal O'Sullivan,Kevin France,Anne Jaskot,Jason Tumlinson,Sanchayeeta Borthakur,Michael Rutkowski,Stephan McCandliss,Ravi Sankrit,John M. O'Meara*

Main category: astro-ph.IM

TL;DR: SPRITE, a CubeSat mission with a far ultraviolet spectrograph, aims to study supernova remnants' interaction with the interstellar medium and ionizing escape fractions from star-forming galaxies, providing critical insights into galaxy feedback and evolution.


<details>
  <summary>Details</summary>
Motivation: To understand the cooling of supernova-heated interstellar medium (dominated by UV emission lines) and the stellar feedback mechanisms crucial for galaxy evolution, as highlighted by the 2020 Astrophysics Decadal Survey. Current observations lack sufficient resolution and wavelength coverage in the FUV to resolve sub-parsec structures in SNR-ISM interactions.

Method: SPRITE uses a long-slit orbital spectrograph with sub-arcminute resolution (10-20 arcseconds) covering 1000-1750 angstroms, including Lyman UV. It employs push-broom mapping of extended sources like SNRs in the LMC, producing 3D data cubes. Simulated observations of LMC SNRs demonstrate instrument efficacy pre-launch.

Result: Results will include maps of key FUV emission lines (O VI, O IV], C IV) at SNR-ISM interfaces, measurements of ionizing escape fractions in low-redshift galaxies, and the first large sample of high-resolution FUV 3D data cubes for SNRs.

Conclusion: SPRITE will revolutionize understanding of stellar feedback processes in galaxies by resolving critical cooling regions and quantifying ionizing radiation escape, directly addressing priorities of the Decadal Survey. Simulations confirm the mission's readiness to achieve these goals.

Abstract: Supernovae (SNe) enrich and energize the surrounding interstellar medium (ISM) and are a key mechanism in the galaxy feedback cycle. The heating of the ISM by supernova shocks, and its subsequent cooling is critical to future star formation. The cooling of the diffuse shock-heated ISM is dominated by ultraviolet (UV) emission lines. These cooling regions and interfaces have complex spatial structure on sub-parsec scales. Mapping this cooling process is essential to understanding the feedback cycle of galaxies, a major goal of the 2020 Astrophysics Decadal Survey. The Supernova remnants and Proxies for ReIonization Testbed Experiment (SPRITE) CubeSat Mission will house the first long-slit orbital spectrograph with sub-arcminute angular resolution covering far ultraviolet wavelengths (FUV; 1000 - 1750 angstroms) and access to the Lyman UV (lambda < 1216 angstroms). SPRITE aims to provide new insights into the stellar feedback that drives galaxy evolution by mapping key FUV emission lines at the interaction lines between supernova remnants (SNRs) and the ambient interstellar medium (ISM). SPRITE will also measure the ionizing escape from approximately 50 low-redshift (0.16 < z < 0.4) star-forming galaxies. Current models predict SPRITE capable of detecting strong O VI, O IV], and C IV emission lines with angular resolution from 10 - 20 arcseconds. The SPRITE SNR survey will use push-broom mapping of its long-slit on extended sources to produce the first large sample of sub-arcminute 3D data cubes of extended sources in the FUV. In this paper, we present simulated SPRITE observations of Large Magellanic Cloud (LMC) SNRs to demonstrate the efficacy of the SPRITE instrument ahead of launch and instrument commissioning. These models serve as critical planning tools and incorporate the final pre-flight predicted performance of the instrument and the early extended source data reduction pipeline.

</details>


### [58] [An Analysis of LIGO Glitches Using t-SNE During the First Part of the Fourth LIGO-Virgo-KAGRA Observing Run](https://arxiv.org/abs/2512.03440)
*Tabata Aira Ferreira,Gabriela González,Osvaldo Salas*

Main category: astro-ph.IM

TL;DR: The paper analyzes noise transients in LIGO's O4a data using t-SNE and clustering, finding that Livingston glitches correlate with ground motion while Hanford's relate to instrumental conditions.


<details>
  <summary>Details</summary>
Motivation: To understand the origin and behavior of noise transients in LIGO observatories by clustering glitches and correlating them with environmental/instrumental factors.

Method: Uses t-SNE for dimensionality reduction of glitch data, applies Agglomerative Clustering with Silhouette Score to determine optimal groups, then tracks groups over time and correlates with environmental conditions.

Result: Identified that Livingston's main glitches were seasonal/ground motion-related, while Hanford's were linked to instrumental conditions.

Conclusion: Environmental and instrumental factors significantly influence glitch types at different LIGO sites, informing future data quality mitigation strategies.

Abstract: This paper presents an analysis of noise transients observed in LIGO data during the first part of the fourth observing run, using the unsupervised machine learning technique t-distributed Stochastic Neighbor Embedding (t-SNE) to examine the behavior of glitch groups. Based on the t-SNE output, we apply Agglomerative Clustering in combination with the Silhouette Score to determine the optimal number of groups. We then track these groups over time and investigate correlations between their occurrence and environmental or instrumental conditions. At the Livingston observatory, the most common glitches during O4a were seasonal and associated with ground motion, whereas at Hanford, the most prevalent glitches were related to instrumental conditions.

</details>


### [59] [Long-term calibration and validation of stability of the Auger Engineering Radio Array using the diffuse Galactic radio emission](https://arxiv.org/abs/2512.03692)
*The Pierre Auger Collaboration,A. Abdul Halim,P. Abreu,M. Aglietta,I. Allekotte,K. Almeida Cheminant,R. Aloisio,J. Alvarez-Muñiz,A. Ambrosone,J. Ammerman Yebra,L. Anchordoqui,B. Andrada,L. Andrade Dourado,L. Apollonio,C. Aramo,E. Arnone,J. C. Arteaga Velázquez,P. Assis,G. Avila,E. Avocone,A. Bakalova,Y. Balibrea,A. Baluta,F. Barbato,A. Bartz Mocellin,J. P. Behler,C. Berat,M. E. Bertaina,M. Bianciotto,P. L. Biermann,V. Binet,K. Bismark,T. Bister,J. Biteau,J. Blazek,J. Blümer,M. Boháčová,D. Boncioli,C. Bonifazi,N. Borodai,J. Brack,P. G. Brichetto Orquera,A. Bueno,S. Buitink,M. Büsken,A. Bwembya,K. S. Caballero-Mora,S. Cabana-Freire,L. Caccianiga,J. Caraça-Valente,R. Caruso,A. Castellina,F. Catalani,G. Cataldi,L. Cazon,M. Cerda,B. Čermáková,A. Cermenati,K. Cerny,J. A. Chinellato,J. Chudoba,L. Chytka,R. W. Clay,A. C. Cobos Cerutti,R. Colalillo,R. Conceição,G. Consolati,M. Conte,F. Convenga,D. Correia dos Santos,P. J. Costa,C. E. Covault,M. Cristinziani,C. S. Cruz Sanchez,S. Dasso,K. Daumiller,B. R. Dawson,R. M. de Almeida,E. -T. de Boone,B. de Errico,J. de Jesús,S. J. de Jong,J. R. T. de Mello Neto,I. De Mitri,D. de Oliveira Franco,F. de Palma,V. de Souza,E. De Vito,A. Del Popolo,O. Deligny,N. Denner,K. Denner Syrokvas,L. Deval,A. di Matteo,C. Dobrigkeit,J. C. D'Olivo,L. M. Domingues Mendes,Y. Dominguez Ballesteros,Q. Dorosti,R. C. dos Anjos,J. Ebr,F. Ellwanger,R. Engel,I. Epicoco,M. Erdmann,A. Etchegoyen,C. Evoli,H. Falcke,G. Farrar,A. C. Fauth,T. Fehler,F. Feldbusch,A. Fernandes,M. Fernández Alonso,B. Fick,J. M. Figueira,P. Filip,A. Filipčič,T. Fitoussi,B. Flaggs,A. Franco,M. Freitas,T. Fujii,A. Fuster,C. Galea,B. García,C. Gaudu,P. L. Ghia,U. Giaccari,C. Glaser,F. Gobbi,F. Gollan,G. Golup,P. F. Gómez Vitale,J. P. Gongora,J. M. González,N. González,D. Góra,A. Gorgi,M. Gottowik,F. Guarino,G. P. Guedes,Y. C. Guerra,L. Gülzow,S. Hahn,P. Hamal,M. R. Hampel,P. Hansen,V. M. Harvey,A. Haungs,T. Hebbeker,C. Hojvat,J. R. Hörandel,P. Horvath,M. Hrabovský,T. Huege,A. Insolia,P. G. Isar,M. Ismaiel,P. Janecek,V. Jilek,K. -H. Kampert,B. Keilhauer,A. Khakurdikar,V. V. Kizakke Covilakam,H. O. Klages,M. Kleifges,J. Köhler,F. Krieger,M. Kubatova,N. Kunka,B. L. Lago,N. Langner,N. Leal,M. A. Leigui de Oliveira,Y. Lema-Capeans,A. Letessier-Selvon,I. Lhenry-Yvon,L. Lopes,J. P. Lundquist,M. Mallamaci,S. Mancuso,D. Mandat,P. Mantsch,A. G. Mariazzi,I. C. Mariş,G. Marsella,D. Martello,S. Martinelli,O. Martínez Bravo,M. A. Martins,H. -J. Mathes,J. Matthews,G. Matthiae,E. Mayotte,S. Mayotte,P. O. Mazur,G. Medina-Tanco,J. Meinert,D. Melo,A. Menshikov,C. Merx,S. Michal,M. I. Micheletti,L. Miramonti,M. Mogarkar,S. Mollerach,F. Montanet,L. Morejon,K. Mulrey,R. Mussa,W. M. Namasaka,S. Negi,L. Nellen,K. Nguyen,G. Nicora,M. Niechciol,D. Nitz,D. Nosek,A. Novikov,V. Novotny,L. Nožka,A. Nucita,L. A. Núñez,S. E. Nuza,J. Ochoa,M. Olegario,C. Oliveira,L. Östman,M. Palatka,J. Pallotta,S. Panja,G. Parente,T. Paulsen,J. Pawlowsky,M. Pech,J. Pękala,R. Pelayo,V. Pelgrims,E. E. Pereira Martins,C. Pérez Bertolli,L. Perrone,S. Petrera,C. Petrucci,T. Pierog,M. Pimenta,B. Pont,M. Pourmohammad Shahvar,P. Privitera,C. Priyadarshi,M. Prouza,K. Pytel,S. Querchfeld,J. Rautenberg,D. Ravignani,J. V. Reginatto Akim,A. Reuzki,J. Ridky,F. Riehn,M. Risse,V. Rizi,E. Rodriguez,G. Rodriguez Fernandez,J. Rodriguez Rojo,S. Rossoni,M. Roth,E. Roulet,A. C. Rovero,A. Saftoiu,M. Saharan,F. Salamida,H. Salazar,G. Salina,P. Sampathkumar,N. San Martin,J. D. Sanabria Gomez,F. Sánchez,F. M. Sánchez Rodriguez,E. Santos,F. Sarazin,R. Sarmento,R. Sato,P. Savina,V. Scherini,H. Schieler,M. Schimassek,M. Schimp,D. Schmidt,O. Scholten,H. Schoorlemmer,P. Schovánek,F. G. Schröder,J. Schulte,T. Schulz,S. J. Sciutto,M. Scornavacche,A. Sedoski,S. Sehgal,S. U. Shivashankara,G. Sigl,K. Simkova,F. Simon,R. Šmída,S. Soares Sippert,P. Sommers,R. Squartini,M. Stadelmaier,S. Stanič,J. Stasielak,P. Stassi,S. Strähnz,M. Straub,T. Suomijärvi,A. D. Supanitsky,Z. Svozilikova,Z. Szadkowski,F. Tairli,M. Tambone,A. Tapia,C. Taricco,C. Timmermans,O. Tkachenko,P. Tobiska,C. J. Todero Peixoto,B. Tomé,A. Travaini,P. Travnicek,C. Trimarelli,M. Tueros,M. Unger,R. Uzeiroska,L. Vaclavek,M. Vacula,I. Vaiman,J. F. Valdés Galicia,L. Valore,P. van Dillen,E. Varela,V. Vašíčková,A. Vásquez-Ramírez,D. Veberič,I. D. Vergara Quispe,S. Verpoest,V. Verzi,J. Vicha,S. Vorobiov,J. B. Vuta,C. Watanabe,A. A. Watson,A. Weindl,M. Weitz,L. Wiencke,H. Wilczyński,B. Wundheiler,B. Yue,A. Yushkov,E. Zas,D. Zavrtanik,M. Zavrtanik*

Main category: astro-ph.IM

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: The Auger Engineering Radio Array (AERA) measures radio emission from high-energy extensive air showers. Consisting of 153 autonomous radio-detector stations spread over $17$\,km$^2$, it detects radio waves in the frequency range of $30$ to $80$\,MHz. Accurate characterization of the detector response is crucial for proper interpretation of the collected data. Previously, this was achieved through laboratory measurements of the analog chain and simulations and measurements of the antenna's directional response. In this paper, we perform an absolute calibration using the continuously monitored sidereal modulation of the diffuse Galactic radio emission. Calibration is done by comparing the average frequency spectra recorded by the stations with predictions from seven different models of the full radio sky, accounting for the system response, which includes the antenna, filters, and amplifiers. The analysis of the calibration constants over a period of seven years shows no relevant and no significant ageing effect in the AERA antennas. This result confirms the long-term stability of the detector stations and demonstrates the possibility for a radio detector to effectively monitor ageing effects of other detectors operating over extended periods.

</details>


### [60] [Lightweight design and analysis of optical cover plate for exoplanet imaging coronagraph](https://arxiv.org/abs/2512.03700)
*Lingyi Kong,Mingming Xu,Wei Guo,Jiangpei Dou,Bo Chen,Shu Jiang*

Main category: astro-ph.IM

TL;DR: The paper presents a carbon fiber optical cover plate design to reduce mass and deformation issues in exoplanet imaging coronagraphs, determining the optimal layup [15/-75/-15/75]s through finite element analysis and dynamic response validation.


<details>
  <summary>Details</summary>
Motivation: To address aluminum alloy cover plate's mass and deformation problems in exoplanet coronagraphs, the study aims to design a lighter, stiffer carbon fiber alternative.

Method: 1. Applied equal strain design method for carbon fiber configuration
2. Performed finite element analysis simulating various layup angles and sequences
3. Evaluated stiffness variations with different layering parameters
4. Conducted dynamic response analysis to validate performance against system requirements

Result: Optimal layup configuration [15/-75/-15/75]s achieved required stiffness and dynamic response specifications, proving carbon fiber's feasibility as a lightweight solution.

Conclusion: The carbon fiber design effectively reduces mass while maintaining structural integrity, demonstrating its viability for exoplanet mission applications.

Abstract: In order to reduce the load mass and solve the problem that the aluminum alloy optical cover plate of exoplanet imaging coronagraph was easy to deform, based on the equal generation design method, this paper designed and determined the configuration of the carbon fiber optical cover plate. Through the simulation of layup by finite element analysis, this paper researched the influence of different layering angles and sequences on the stiffness of optical cover plate. Finally, the carbon fiber layup method was determined as [15/-75/-15/75]s. The dynamic response analysis show that all the indexes satisfy the system requirements, and verify the feasibility of carbon fiber optical cover plate.

</details>


### [61] [Gamma-ray astronomy from the ground - future perspectives](https://arxiv.org/abs/2512.03798)
*Jim Hinton*

Main category: astro-ph.IM

TL;DR: The paper discusses the future of ground-based gamma-ray astronomy, emphasizing the complementary roles of particle detector arrays and Cherenkov telescope arrays. It highlights ongoing projects like CTAO and SWGO, and emerging concepts such as the plenoscope and lake-based instruments for future advancements across energy ranges.


<details>
  <summary>Details</summary>
Motivation: To address the future scientific goals in gamma-ray astronomy, the paper outlines the need for advanced instrumentation that combines wide-field detection capabilities with precise imaging. This integration is critical for exploring key astrophysical phenomena across a broad energy spectrum.

Method: The author provides a personal perspective reviewing current projects (CTAO, SWGO) and emerging technologies (plenoscope, lake-based arrays), evaluating their roles in advancing gamma-ray detection capabilities and covering the full sky across low to ultra-high energies.

Result: The analysis underscores the necessity of both particle detector arrays and Cherenkov telescopes for optimal progress. It identifies potential long-term advancements through next-generation projects and concepts, aiming to enhance sensitivity and energy range coverage in the field.

Conclusion: Future gamma-ray astronomy will rely on the synergy between existing and emerging technologies. Prioritizing these projects ensures comprehensive exploration of high-energy astrophysical processes, with potential breakthroughs in understanding cosmic phenomena.

Abstract: I provide a personal perspective on the future of the field of ground-based gamma-ray astronomy, on the occasion of the 2024 {\it Gamma} conference in Milan. I discuss some of the scientific motivations for new instrumentation and the major new projects that are in development or already under construction, together with emerging concepts for instrumentation in the farther future. I stress the strong complementarity of the ground-level particle detector arrays, with their wide-field capabilities, and the more precise Cherenkov telescope arrays. The key science topics for the next decades require both approaches and both are developing rapidly towards major performance advances and full sky coverage. I will briefly outline the status and roles of the projects CTAO and SWGO which will dominate the next decade. Beyond these projects are several developments which might boost performance at both ends of the ground-based gamma-ray energy range, including the plenoscope approach at low energies and diverse approaches to ultra-high-energy gamma-ray astronomy; from lake-based instruments to arrays of very small Cherenkov telescopes. I will again briefly review these activities and how they may contribute long term.

</details>


### [62] [Large Language Models for Limited Noisy Data: A Gravitational Wave Identification Study](https://arxiv.org/abs/2512.04031)
*Yixuan Li,Yuhao Lu,Yang Liu,Liang Li,R. Ruffini,Di Li,Rong-Gen Cai,Xiaoyan Zhu,Wenbin Lin,Yu Wang*

Main category: astro-ph.IM

TL;DR: This paper shows that fine-tuned LLMs achieve high accuracy (97.4%) in identifying gravitational wave signals with limited data and non-Gaussian noise, outperforming traditional neural networks which require more simulated data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to determine if LLMs have advantages over traditional networks in handling astronomical data with non-Gaussian/non-stationary noise and small labeled datasets.

Method: The authors used 90 LIGO gravitational wave events to fine-tune LLMs and compared performance against traditional neural networks. They also tested the effect of additional simulated data and model/dataset scaling.

Result: LLMs achieved 97.4% accuracy without improvement from additional simulations, unlike traditional methods. Performance scaled predictably with larger models/datasets.

Conclusion: LLMs can effectively extract discriminative features from real observational data with limited samples, offering efficiency for gravitational wave and potentially other astronomical signal identifications.

Abstract: This work investigates whether large language models (LLMs) offer advantages over traditional neural networks for astronomical data processing, in regimes with non-Gaussian, non-stationary noise and limited labeled samples. Gravitational wave observations provide an suitable test case, using only 90 LIGO events, finetuned LLMs achieve 97.4\% accuracy for identifying signals. Further experiments show that, in contrast to traditional networks that rely on large simulated datasets, additional simulated samples do not improve LLM performance, while scaling studies reveal predictable gains with increasing model size and dataset size. These results indicate that LLMs can extract discriminative structure directly from observational data and provide an efficient assessment for gravitational wave identification. The same strategy may extend to other astronomical domains with similar noise properties, such as radio or pulsar observations.

</details>
